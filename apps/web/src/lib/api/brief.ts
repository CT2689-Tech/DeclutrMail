 ...
 * the snapshot worker hasn't fired yet (the FE renders an empty / "Brief
 * lands soon" state in that case; the snapshot worker ticks every hour
 * per D69 so a refetch on focus picks it up).
 */
