/**
 * Sender Detail fixture builder — stories and mock handlers ONLY.
 *
 * Moved out of `features/senders/detail/data.ts` (2026-07-16 wire
 * unification) so the prod module keeps only the formatting helpers
 * the detail components consume. Everything here is fixture-only
 * heuristics over the ergonomic `SenderFixture` authoring shape; the
 * built detail's `sender` field goes through the SAME
 * `fixtureToSenderListRow` → `enrichSenderRow` seam as live data.
 *
 * Fixtures are deterministic (seeded by sender id) so Storybook
 * variants stay stable across builds.
 */

import { enrichSenderRow } from '../features/senders/data';
import type {
  DecisionHistoryRow,
  ProtectionReason,
  Recommendation,
  ...
  TimeseriesPoint,
  Verdict,
} from '../features/senders/detail/types';
import type { SenderFixture } from './sender-fixture-data';
import { fixtureToSenderListRow } from './sender-fixtures';

type SenderGroup = SenderFixture['group'];
...
function inferVerdict(s: SenderFixture, isProtected: boolean): Verdict {
...
function inferConfidence(s: SenderFixture, verdict: Verdict): number {
...
function buildReasoning(s: SenderFixture, verdict: Verdict): string {
...
function buildSignals(s: SenderFixture): string[] {
  const out = [
...
function buildRecommendation(s: SenderFixture, isProtected: boolean): Recommendation | null {
...
function buildRecentMessages(s: SenderFixture): RecentMessage[] {
...
function buildStats(s: SenderFixture): SenderStats {
  return {
...
function buildTimeseries(s: SenderFixture): TimeseriesPoint[] {
...
function buildHistory(s: SenderFixture): DecisionHistoryRow[] {
...
/**
 * Build a complete fixture `SenderDetail` from a fixture + posture
...
export function buildSenderDetail(
  ...
  const isProtected = overrides.isProtected ?? fixture.protected === true;
  const protectionReason: ProtectionReason | null = isProtected
    ? (overrides.protectionReason ?? (fixture.protected ? 'starred' : 'user-marked'))
    : null;
  ...
  return {
    sender,
    // Wire-row email — keeps the detail header and the list row in
    // agreement for the same fixture.
    email: row.email,
    ...
    gmailCategory: GMAIL_CATEGORY[fixture.group],
    isProtected,
    protectionReason,
    recommendation: buildRecommendation(fixture, isProtected),
    recentMessages: overrides.recentMessages ?? buildRecentMessages(fixture),
    stats: buildStats(fixture),
    timeseries: buildTimeseries(fixture),
    history: overrides.history ?? buildHistory(fixture),
  };
}
