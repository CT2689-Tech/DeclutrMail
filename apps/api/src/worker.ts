  ...
  async function sweepBillingReconciliation(): Promise<void> {
    ...
    try {
      ...
      console.error(
        JSON.stringify({
          level: 'error',
          kind: 'billing.reconcile.sweep_failed',
          message: error.message,
        }),
      );
      observer.captureBackgroundFailure(error, { kind: 'billing.reconcile.sweep_failed' });
    }
  }
  void sweepBillingReconciliation();
  const billingReconcileHandle = setInterval(() => {
    void sweepBillingReconciliation();
