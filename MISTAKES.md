...
## 2026-09-03 — Gmail quota limiter started every mailbox's bucket FULL, letting a fresh sync burst the whole per-minute budget in one shot
**PR:** TBD (fix built this session, not yet merged)
**Caught by:** founder manual prod query (`provider_sync_state.readiness_status='failed'`, workspace `300563ea-…`) — no automated check existed for this state at all; see the companion `check-sync-stuck.sh`/`check-sync-failed.sh` entry this session for that half.
**What happened:** Two real signups (2026-08-26 and 2026-08-06; their addresses were redacted from this file on 2026-09-26) hit `RateLimitError` on their first `InitialSyncWorker` run and never onboarded. The first user's dead-letter jobs show the mechanism precisely: 4 separate manual "Try again" retries over 24 minutes, each opening with the SAME instant burst, escalating rather than clearing — cycle 1 failed 15 calls into a fetch chunk, cycle 4 (20 minutes and 3 retries later) failed on the very first Gmail call of the job (`getProfile`). A per-minute token bucket idle for 6–9 minutes between cycles should have fully refilled; it hadn't. Root cause: `RedisGmailQuotaLimiter` (and its in-process `RateLimiter` fallback) computed ONE number — `capacity` — as both the bucket's ceiling AND the basis for its refill rate, so a bucket sized for a conservative 12,000-units/60s SUSTAINED average also started every fresh mailbox with all 12,000 units already sitting in it. `InitialSyncWorker`'s fetch loop (`FETCH_CONCURRENCY=20`, back-to-back chunks) could and did spend the entire minute's budget — ~2,400 `messages.get` calls — in a single burst before either limiter ever introduced a millisecond of pacing. A large mailbox's opening burst is consistent with tripping a Gmail-side burst penalty with a cooldown measured in minutes rather than the documented 60s per-minute window, and each retry's own opening burst plausibly re-armed that penalty before the previous one expired — a self-perpetuating lockout, not a one-off blip. Separately (and independently fatal on its own): `perMailboxPolicy`'s retry backoff was a fixed 2s/4s/8s/16s schedule regardless of error type — ~30s total across 5 attempts, structurally unable to outlast a 60s quota window even for a mailbox that DIDN'T trip the longer burst penalty — see the companion "backoff blind to RateLimitError.retryAfterMs" entry this session.
**Correct approach:** Decouple burst ceiling from sustained rate as two independent knobs on the SAME token bucket (the Lua script already took `capacity` and `refill_per_ms` as separate ARGVs — the coupling was entirely in the JS constructor deriving both from one number). `RedisGmailQuotaLimiter`'s constructor now takes `burstCapacity` (how much may be spent instantly) separately from `sustainedUnitsPerWindow`/`windowMs` (refill rate only). Production wiring (`apps/api/src/worker.ts`) sets `GMAIL_QUOTA_BURST_CAPACITY=1_000` (200 calls — 10 `FETCH_CONCURRENCY` chunks) against the unchanged `GMAIL_QUOTA_UNITS_PER_MIN=12_000`/`GMAIL_QUOTA_WINDOW_MS=60_000` sustained pair — same long-run throughput (verified live: 360 calls admitted in 4s against an old-behavior expectation of 2,400 admitted instantly), just paced instead of front-loaded. The in-process `RateLimiter` fallback got the equivalent treatment: `RateLimiter(1_000, 5_000)` is mathematically the SAME 200-units/sec sustained rate as `RateLimiter(12_000, 60_000)` (a sliding window's "maxUnits per windowMs" already IS both burst and average in one pair, so shrinking both proportionally caps burst without changing steady throughput) — without this the fallback would silently revert to full-burst behavior the instant Redis degraded.
**Rule:** A token bucket (or sliding window) constructed from a single `(capacity, windowMs)` pair conflates two genuinely different questions — "how much may be spent in one instant" and "what's the steady-state average over time" — and a caller that fires many concurrent requests per acquire cycle (any bulk/backfill loop) will exploit exactly that conflation on its very first run, before the limiter has ever had a chance to demonstrate steady-state behavior. Any rate limiter backing a bulk external-API loop needs burst capacity sized to roughly one batch/chunk of the caller's own concurrency, not to the full window's total budget — decouple the two explicitly rather than assuming "conservative average" implies "safe burst."
**Enforcement update:** `gmail-quota-limiter.test.ts` gained a dedicated burst-vs-sustained test against the real Lua script + real Redis (not mockable — a mocked `evalsha` would pass regardless of whether the two are actually decoupled), plus a live throwaway smoke replicating `InitialSyncWorker`'s exact chunk pattern against the real production constants before shipping. No hook enforces the *choice* of burst-capacity value going forward — that is a product/capacity judgment call, not a fixed-string check; see CLAUDE.md §8's "a hook can only enforce a match, not a reading."

## 2026-09-19 — Browser React invariant errors reached Sentry with no message, tag or URL — captured, transported, unfindable
**PR:** #746 (https://github.com/CT2689-Tech/DeclutrMail/pull/746)
**Caught by:** manual — a session saw `__sentry_captured__: true` on a production `#418` and a Sentry search for its message returned nothing.
**What happened:** The browser `beforeSend` rebuilds every event through `scrubSentryEvent`, which drops `exception.value` by design (D7). The server profile was given fingerprints and class names on 2026-08-21 for exactly this reason; the browser profile was not. React throws every invariant as a bare `Error` from one internal frame, so a hydration failure arrived as an untitled `Error` grouped with every other React invariant, carrying nothing that says "hydration" or where. A capture path that cannot be queried is the §8 "guard that cannot fail" shape: the dashboard read clean whether or not the error was happening.
...
**PR:** #750 (https://github.com/CT2689-Tech/DeclutrMail/pull/750)
...
## 2026-09-20 — Made the pill the ONLY voice, then left three ways for it to say nothing
**PR:** feat/d226-panel-progress-one-channel (one-pill panel + button-becomes-status)
**Caught by:** flow-completeness-auditor + design-system-agent + architecture-guardian (pre-PR), and my own live smoke
**What happened:** Retiring the Senders/Detail top strip, the outcome toasts and the inline Undo moved every outcome onto one bottom pill. I verified the pill's happy path and shipped it to the gates with: (1) the tray's per-screen baseline still hiding any decision that existed when a screen was entered — so opening a sender right after acting swallowed the only Undo left; (2) `retry:false` + "stop polling on error" freezing "Archiving…" forever after one 5xx; (3) the row un-mark listener keyed on `variables.token` while the pill's per-sender Undo sends `memberToken`, so a row kept saying "Archived" for mail that was back; (4) a live region mounted together with its first message (not reliably announced) as the sole screen-reader channel; (5) an inline `animation:` spinner that outranks the reduced-motion rule `tokens.css` documents; (6) an API type asserting `'archive'|'later'|'delete'` over a column that also holds `'unsubscribe'`. My own smoke found a seventh: below ~1300px the table scrolls sideways and the action column — now the status — was off-screen.
...
## 2026-09-25 — One bulk Delete held a user's mailbox lock for 8 minutes and dead-lettered eight redundant syncs
**PR:** not yet opened (fix prepared in session 2026-09-24 PDT)
**Caught by:** production — Cloud Run logs (`mailbox_lock.acquire_failed`, `worker.incremental.terminal_failed`), mailbox `ref_f55fc2a50d75689402e2df84`
**What happened:** A new user's bulk Delete (8,799 messages, 21 action jobs) produced a Pub/Sub push per Gmail label change. `ensureIncrementalSyncJob` keyed jobs on `${mailbox}__${endHistoryId}`, so ten incremental jobs ran for one mailbox in nine minutes. One applied 8,692 label records one UPDATE at a time under the mailbox advisory lock: `lockHoldMs 482432`, ~55ms per record, which is two cross-region round trips per statement (postgres.js describes unprepared statements before executing them). The other eight re-read the whole burst from Gmail on every attempt, waited out the 45s lock timeout five times each, and dead-lettered; five label-action jobs failed attempt 1 on the same timeout. Reproduced locally through a 24ms-RTT proxy: 512s hold, 8,708 statements.
...
## 2026-09-26 — A single half-open probe turned the first sweep after a top-up into templates
**PR:** branch `fix/d024-llm-rejection-breaker` (caught before the PR was opened)
**Caught by:** my own smoke — real SDK over HTTP to a local server returning the production credit-balance 400, real adapters, real ScoreWorker (4 calls in flight, 400/min pacing) on PGlite
**What happened:** The first version of the LLM refusal breaker admitted exactly one probe call after the cool-down and reported "blocked" to everyone else while it was in flight. Every unit test passed. In a real sweep, senders that see "blocked" skip the call and finish in microseconds, so the whole queue drained behind one probe's network latency: with credits restored, the next sweep wrote 1 LLM reason and 199 templates. The design was right for one caller and wrong for a pool of concurrent ones.
**Correct approach:** Drop the exclusive probe. The pause is a timestamp: while it runs, calls are skipped; after it, calls simply go out, and the first refusal re-pauses. Worst case is one refused call per in-flight slot per cool-down, and refused calls are not billed. Pinned by `llm-circuit-breaker.sweep.spec.ts` (real ScoreWorker + real adapter; the old breaker fails it 1 LLM / 19 blocked).
**Rule:** A gate that makes waiters skip instead of wait must be exercised under the real consumer's concurrency — a skipped item completes instantly and the queue races past the gate.
**Enforcement update:** the sweep-level regression test; no hook.

## 2026-09-26 — The fail-closed alert I was asked for listed the bad shapes it knew, and passed the rest
**PR:** branch `fix/d024-llm-rejection-breaker` (fixed before the PR was opened)
**Caught by:** silent-failure-hunter, typescript-reviewer, privacy-auditor and architecture-guardian (pre-PR, no BLOCKING; every finding folded in)
**What happened:** The founder asked for an LLM-refusal page that fails closed per CLAUDE.md §8. My verify step checked the policy for four known-bad shapes, so a disabled metric, a condition narrowed to one reason, `trigger: {count: 5}`, a second AND-ed condition or an active snooze all printed "Wired". Same pattern in the classifier: a 429 spend cap without its structured code fell through as a rate limit — no line, no page, the 09-24 incident again. Its unanchored wording regexes would have read an echoed Brief snippet ("your credit balance is…") as a credit refusal and paused both adapters. Provider `type`/`error_code`/`request_id` were logged raw, though the same body's `message` was dropped for exactly that echo risk. The Brief worker lost its per-mailbox failure line and nothing replaced it.
...
## 2026-09-26 — Both stuck-mailbox guards were already red, so a newly stuck mailbox changed nothing
**PR:** #778 (https://github.com/CT2689-Tech/DeclutrMail/pull/778)
**Caught by:** founder review of alert state (Cloud Monitoring alert above zero since it was created 2026-09-05; `sync-stuck-watchdog` red on 155 scheduled runs in a row from 2026-09-03 23:21 UTC)
**What happened:** Both guards reduced "which mailboxes are stuck" to one bit. The Cloud Monitoring policy summed every `mailbox.stuck_unnoticed` line into one series (`REDUCE_SUM`, no `groupByFields`), so its single alert opened for the first stuck mailbox and stayed open while three mailboxes stayed stuck; a fourth would only have raised a number inside an alert that had already paged. `sync-stuck-watchdog.yml` failed every run for the same two first-scan failures, so its red carried no news either. Neither could fail when its own input vanished: a sweep that stopped running read as "nothing stuck". The workflow header also promised a 5-minute cadence; GitHub started it 5-8 times a day.
**Correct approach:** Make the unit of alerting the unit of news. The metric carries `mailbox_account_id` + `reason` labels and the policy opens one alert per pair, closing it when the line stops (`EVALUATION_MISSING_DATA_INACTIVE`), so known ones can be acknowledged while new ones still page. The sweep ends with `stuck_mailbox_watchdog.completed`, and a PromQL `absent_over_time` policy pages when it stops (a native metric-absence condition needs one point before it can ever fire). The GH watchdog fails only for rows missing from `scripts/known-stuck-mailboxes.tsv`, matched on mailbox + stuck-since instant so a re-failure is new.
**Rule:** A guard over a set alerts per member (or per unacknowledged member), never on the set being non-empty — once one member is stuck forever, a non-empty check has stopped testing anything.
**Enforcement update:** `scripts/stuck-mailbox-alert.test.mjs` (per-mailbox grouping, close on recovery, PromQL absence, worker log contract, starve lines filtered from prod), `scripts/check-known-stuck.test.mjs` (fresh-beside-known red, re-failure red, missing/malformed list exit 2, committed list valid; run in CI), `reportStuckMailboxes` specs — each verified red against a negative control.

## 2026-09-26 — Two real signups' email addresses sat in the public repo for three weeks
**PR:** #722 introduced them (2026-09-03); fixed by fix/d007-redact-signup-addresses
**Caught by:** manual review (the stuck-mailbox alert session, reading MISTAKES.md for context)
**What happened:** An incident entry and a code comment quoted two signups' Gmail addresses verbatim while documenting their failed first scans. The repo is public, so both addresses were readable by anyone from 2026-09-03 until this fix; they remain in git history.
