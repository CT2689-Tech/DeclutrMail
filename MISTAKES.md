...
**Correct approach:** (1) restrict the job to same-repo PRs (`head.repo.full_name == github.repository`); (2) execute the TRUSTED base-ref copy of the helper (`git show origin/$BASE:…`), never the PR's working-tree copy, with a fallback only on the introducing PR; (3) drop the sandbox bypass — run `codex exec -c sandbox_mode=read-only -c approval_policy=never` so an injected diff cannot drive command execution. Verified locally that sandboxed codex still returns the review JSON.
