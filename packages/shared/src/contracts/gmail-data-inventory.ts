...
export const GMAIL_MESSAGE_DATA_INVENTORY = [
  ...
  {
    id: 'gmail-label-names',
    category: 'message',
    label: 'Your Gmail label names',
    fetchedFrom: ['labels.list'],
    storageRefs: ['mailbox_labels.name', 'mailbox_labels.label_id'],
    derived: false,
    purpose:
      'Recognise labels applied by third-party inbox tools, so read counts reflect what you read rather than what a tool marked.',
    retention: GMAIL_DATA_RETENTION.mailboxIndex,
    removalTrigger: 'delete-indexed-data',
    exportedIn: ['json'],
    transmittedTo: ['DeclutrMail'],
  ...
  },
  {
    ...
    derived: true,
    purpose:
      'Show engagement facts and calculate read rates for each sender, excluding messages a third-party inbox tool marked read.',
    retention: GMAIL_DATA_RETENTION.mailboxIndex,
    removalTrigger: 'delete-indexed-data',
    exportedIn: ['json', 'csv'],
    transmittedTo: ['DeclutrMail', 'Anthropic'],
