...
export const ACTION_SAFETY_SUMMARY =
  'Manual sender cleanup in Triage and Senders shows a current-scope preview before mail moves. Manual Archive and Later can be reversed from Activity for your plan’s undo window. Delete has an Activity undo token for up to 30 days while Gmail still retains the message; emptying Trash can end recovery sooner. A delivered unsubscribe request cannot be recalled. Observe-mode Autopilot approvals show the sender scope; enabled Pro rules apply future matches without a new per-message approval.';
...
export const DELETE_RECOVERY_CLAIM =
  'Delete moves mail to Gmail Trash. Gmail normally retains it for up to 30 days, but permanently deleting a message or emptying Trash can end recovery sooner.';
