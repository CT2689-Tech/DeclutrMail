 ...
 * calls one of these two helpers —
 *
 *   - `InitialSyncWorker` / `IncrementalSyncWorker` check
 *     `isSyncPausedForDeletion` at processJob entry — the authoritative
 *     backstop that covers every producer (webhook, drift sweep,
 *     "Sync now", connect-time enqueue).
