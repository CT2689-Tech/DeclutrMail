...
**Status:** **Done 2026-07-26** — both halves. Founder set `GCP_BILLING_ACCOUNT_ID` and granted `roles/billing.viewer` to `declutrmail-deploy@` **on the billing account**. First-ever green reading, same day: `Google Cloud (budgets) 🟢 OK — budgets armed — declutrmail-pre-launch-30: 20 USD`. All 9 vendors now report. Two WARNs remain and neither is this item: Vercel (recurring transient API timeout) and GitHub Actions (11,470 min MTD of 2,000 included = 574%, net spend $0.00 — worth understanding why it is free). **Reminder carried forward:** `declutrmail-pre-launch-30` is an alert threshold, not a hard cap, and its $20 amount does not match its name; the 2026-06-08 "Hard $60/mo billing cap" item is the real protection and is still open. Detail below.
