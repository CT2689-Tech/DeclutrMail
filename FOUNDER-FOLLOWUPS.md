...
### 2026-07-30 — The derived impl-log gate is unsatisfiable by the PR that causes the drift
**Source:** session 2026-07-30 (D249 CI triage)
**Why:** `generate-impl-log` derives rows from `gh pr list --state merged` — **merged PRs only**, with no awareness of the PR being checked. So for any PR that closes a new D-row: while it is open its own trailer is unmerged, the generator excludes it, and the branch must commit that D-less output to pass the gate; the instant it merges, `main` is stale by exactly that row and **the next PR opened fails** — whoever that is, including docs-only ones. Not occasional drift: a structural certainty, one failure per D-closing merge. Observed live — #435's merge left five rows stale (D35/D117/D119/D162/D218), failing CI on both #436 and #437, each of which had to carry an unrelated mechanical commit; #436 closes D249 (absent from the log today) and will do the same to the next author. The two regeneration commits pay #435's debt and do not break the cycle. Separately, the rolled-up `Test` check reports red purely because it aggregates the impl-log job — every real shard passed, so "Test failed" here is misleading.
**How:** **not** by re-adding a post-merge push to `main` — `ci.yml:131-134` records that `pr-merged.yml` did exactly that and branch protection rejected it on EVERY run, never once writing a flip (D158, 2026-07-28). Viable options: (a) **teach the generator to count the checked PR's own `Closes D###` trailers** when running in PR context, so the branch commits the post-merge-correct log and `main` is never stale — the only fix that makes the gate satisfiable by the PR causing the drift; (b) pair (a) with CI committing the regenerated file to the PR's **own** branch (feature branches are unprotected, so no bypass credential is needed) to remove the manual step; (c) grant a GitHub App/PAT bypass to push `main` — reintroduces what branch protection deliberately blocked, plus a credential that can write `main`; (d) make the gate advisory and let a scheduled job open a repair PR; (e) stop calling the log derived and maintain it by hand. Recommend (a), optionally with (b). (c) is the one to avoid.
**Verifies by:** merge a PR that closes a new D-row, then open a throwaway PR — its "Implementation log is derived and current" check passes with no impl-log commit on the throwaway branch.
**Status:** Open
