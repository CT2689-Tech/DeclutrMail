...
| Activity | Retry a failed action directly from its row using a fresh preview | Turns Activity into the recovery center instead of a passive log | **Implemented 2026-07-14:** failed Archive/Later/Delete rows verify label-only provider state, disclose partial/already-applied/missing outcomes, and append an idempotent linked attempt only after confirmation. Enqueue acknowledgement loss heals under the same key; reconnect is actionable; unsubscribe has no generic retry. |
| Activity | Exportable action timeline with human-readable support bundle | Helps users and support diagnose provider or automation issues | **Implemented 2026-07-14:** a review dialog starts from the current 30-day/filter state and streams every matching row into a ZIP with a human summary and CSV. Sender addresses are masked by default; full addresses and the strict identifier/status appendix are independent explicit options. |
