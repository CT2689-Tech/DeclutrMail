...
| Reader → owner                                                    | Table                        | Why exception is pragmatic at launch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| senders → triage                                                  | `triage_decisions`           | Senders Detail shows the engine's current verdict as a labelled SUGGESTION (`SenderDetail.recommendation`) and the Senders list shows its last-decision columns. Read-only; event-projection would require a new `senders_decision_history` table + dual-write before the access pattern is well-understood. NOTE (2026-08-19): this exception no longer covers the D46 decision history — serving `triage_decisions` rows there made untouched senders claim decisions the user never made, so that endpoint moved to `activity_log` (row below).                                                                                                                                         |
| senders → activity (shared audit table)                           | `activity_log`               | The D46 decision history is "what happened to this sender", and `activity_log` is where every feature records that: senders itself WRITES to it (the D43 Protect audit rows in `SendersPolicyService`), as do actions, followups and the workers. One sender-keyed read returns the whole decision trail and keeps Sender Detail and the Activity feed answering from identical rows — the disagreement that a separate projection would reintroduce is exactly the defect this replaced. Added 2026-08-19.                                                                                                                                                                                |
| senders → messages (implicit, currently owned by the sync worker) | `mail_messages`              | Senders Detail's recent-messages section is the natural Senders-table join — every message is keyed by `sender_key` and a per-sender list is the fastest path. There is no separate "messages" feature module today; the sync worker writes the table and senders is the first reader. When a messages feature module materializes, this exception is the migration's first candidate.                                                                                                                                                                                                                                                                                                     |
| triage → senders                                                  | `sender_policies`            | Every Triage queue row carries the D245 protection flag + reason (a `leftJoin` in `listQueue`), and the protection review's counts/weak-keys reads (`readProtectionReview`) are pure policy-state questions asked while composing a triage read. Read-only; projecting protection state into a triage-owned table would dual-write a safety flag, which is worse than the crossing. Ratified 2026-08-10 (post-merge review of #483).                                                                                                                                                                                                                                                       |
| actions → senders                                                 | `senders`, `sender_policies` | Every action resolves a sender id to its `sender_key` (ownership) and reads `unsubscribe_method` to decide which unsubscribe path it may take — the capability IS the routing input, so it cannot be projected after the fact. `sender_policies` is read for the D42 Protected guard and written as the transitional dual-write beside the `actions.unsubscribe_intent_recorded` event: the event is the permanent channel, the upsert is the backstop that keeps the sender chip honest until the dispatcher is confirmed in prod (FOUNDER-FOLLOWUPS 2026-06-06), and both retire together. Marked at every call site in `actions.service.ts`. Ratified 2026-08-10 (D248 review of #500). |
