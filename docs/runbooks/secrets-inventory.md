...
| Prod DB URL   | GCP Secret Manager: `database-url-prod` version 2 — real Supabase pooled DSN (`aws-0-us-west-2.pooler.supabase.com:6543`, transaction-mode). Migration DSN (session pooler, port 5432) stays out of the runtime mount; founder runs `atlas migrate apply` locally. Note: the WORKER derives the session-pooler DSN from this same secret at runtime (port rewrite, PR #509) for its advisory-lock + outbox LISTEN pools — no second secret exists. ADR-0022 documents this swap from Cloud SQL → Supabase. | `DATABASE_URL` (Cloud Run API + worker) | 2026-06-08                                                         | founder |
...
headers. On BOTH services by necessity: the worker signs at enqueue
time, the API verifies Gmail's POST. ≥32 chars; generate with
`openssl rand -base64 48`. **Rotating it invalidates the unsubscribe
links in every already-delivered email** — rotate only on suspected
leak, not on cadence.
