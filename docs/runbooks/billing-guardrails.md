...
| **Vercel**          | Bandwidth, function invocations, builds                    | **Pro (as of at least 2026-08-29)** — Spend Management not yet turned on, see FOUNDER-FOLLOWUPS 2026-08-29                                           | Pro: Settings → Billing → Spend Management (50/75/100% emails) — not yet enabled    | Static-leaning frontend; no server $ paths                                                                       | `GET /v1/billing/charges` `BilledCost` sum · `VERCEL_TOKEN` + `VERCEL_TEAM_ID` — **currently UNCONFIGURED, neither secret is set**, so this row has not run since the Pro upgrade                                                            | founder |
...
| Vercel         | **Pro** (as of at least the Aug 29–Sep 28, 2026 cycle) | Spend Management NOT yet turned on — see FOUNDER-FOLLOWUPS 2026-08-29                     | **$59.99** for the Aug 29–Sep 28 cycle (Pro seat $20 + usage: Build CPU Minutes, Function Invocations, Fluid CPU/Memory, Fast Origin Transfer, ISR Reads, Observability Events) — this row was stale "Hobby / $0" until 2026-08-29 |
...
2. ```bash
   ...
   ```
   The `/v1/billing/charges` endpoint is team-scoped. **Overdue:** the
   team is already on Pro (confirmed by a real $59.99 receipt for the
   Aug 29–Sep 28, 2026 cycle) and these secrets were never wired, so
   `check-vendor-limits.mjs` has been reporting Vercel as
   `UNCONFIGURED` — a silent skip, not a failure — through at least
   that whole cycle. See FOUNDER-FOLLOWUPS 2026-08-29.
