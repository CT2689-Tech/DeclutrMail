...
| **Vercel**          | Bandwidth, function invocations, builds                    | **Pro (confirmed by watchdog runs since at least 2026-08-23)** — Spend Management NOT yet turned on, see FOUNDER-FOLLOWUPS 2026-08-29                | Pro: Settings → Billing → Spend Management (50/75/100% emails) — not yet enabled    | Static-leaning frontend; no server $ paths                                                                       | `GET /v1/billing/charges` `BilledCost` sum · `VERCEL_TOKEN` + `VERCEL_TEAM_ID` — **both ARE set and the check runs correctly**; it has been WARN since 2026-08-23 and BREACH (failing the daily job) since 2026-08-28                                                                             | founder |
...
| Vercel         | **Pro** (confirmed by the watchdog since at least 2026-08-23) | Spend Management NOT yet turned on — see FOUNDER-FOLLOWUPS 2026-08-29                                                                   | **BREACH**: $46.81 MTD as of 2026-08-29 (234% of the $20 warn threshold); WARN since 2026-08-23, BREACH since 2026-08-28 — the daily watchdog job has been failing on this for a week; matches the forwarded $59.99 receipt for the Jul 29–Aug 28 usage cycle + Aug 29–Sep 28 plan prepay |
...
2. ```bash
   ...
   **Corrected 2026-08-29:** this step previously said "wire both
   ...
   What's actually missing is (b) below: nothing caps Vercel spend at
   the vendor side, so the daily watchdog failing (which it has, every
   day since 2026-08-28) is the ONLY thing standing between real usage
   and an unbounded bill. Re-run this step only if rotating the token.

**(b) Built-in cap** — **not yet turned on; do this now, it is not
optional once real usage is billing.** Dashboard → Settings →
