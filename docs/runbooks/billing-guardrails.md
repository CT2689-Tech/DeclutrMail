...
| **Vercel**          | Bandwidth, function invocations, builds                    | **Pro (confirmed by watchdog runs since at least 2026-08-23)** — Spend Management IS on: $40 on-demand budget, notifications on, Pause Production Deployments on (confirmed by founder screenshot 2026-08-29) | Pro: Settings → Billing → Spend Management — **confirmed live**, $40 cap, notifications + auto-pause both on | Static-leaning frontend; no server $ paths                                                                       | `GET /v1/billing/charges` `BilledCost` sum · `VERCEL_TOKEN` + `VERCEL_TEAM_ID` — **both ARE set and the check runs correctly**; it BREACHed (failing the daily job) on the closed Jul29–Aug28 cycle, current Aug29–Sep28 cycle reads $0 so far                                                    | founder |
...
What we are actually on today, cross-checked against the live watchdog
output on 2026-08-29 (see run history at
`https://github.com/CT2689-Tech/DeclutrMail/actions/workflows/vendor-limits-watchdog.yml`).
Update this table in the same PR as any plan change — most of the rows
below were stale until this pass; verify against a live run, not memory.

| Vendor         | Plan                                                          | Limit that matters                                                                                                                                                                                                                                                                                        | Cost today                                                                                                                                                                                                                                                        |
| -------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Upstash Redis  | Fixed 250MB                                                   | 10K commands/sec rate limit · 250MB storage · 50GB bandwidth/mo — all throttle, none bill                                                                                                                                                                                                                 | $9.36 spent MTD as of 2026-08-29, projecting $10.10 against the $30 database budget (34% of the warn gauge) — OK                                                                                                                                                  |
| Anthropic      | Tier 2                                                        | $500/mo invoice limit · 1000 RPM `claude-haiku-4-5` · per-slot key caps $20/$50/$100                                                                                                                                                                                                                      | **Untracked by the daily watchdog until 2026-08-29** — no `checkAnthropic()` existed in `check-vendor-limits.mjs` despite this table long implying one did; now implemented, `UNCONFIGURED` until `ANTHROPIC_ADMIN_KEY` is created (FOUNDER-FOLLOWUPS 2026-08-29) |
| GCP            | Pay-as-you-go                                                 | Budget `declutrmail-pre-launch-30` — **live watchdog reads it as a $20 budget, not $30**; reconcile which is correct in the GCP console                                                                                                                                                                   | budgets armed, alert-only, never stops spend — OK per watchdog                                                                                                                                                                                                    |
| Supabase       | Free                                                          | 500MB DB · 2GB egress/mo · pauses idle projects after 7d                                                                                                                                                                                                                                                  | 341.0 MB / 400 MB warn (85%) as of 2026-08-29 — OK but close to the warn line, worth a look                                                                                                                                                                       |
| Vercel         | **Pro** (confirmed by the watchdog since at least 2026-08-23) | Spend Management **IS on** — $40 on-demand budget, notifications on, Pause Production Deployments on (founder screenshot, 2026-08-29). The watchdog's own $20 `VERCEL_MTD_COST_WARN_USD` is a separate, lower, software-only threshold — raise it or accept it stays noisier than the real $40 vendor cap | Closed Jul29–Aug28 cycle: BREACHed the watchdog's $20 warn line ($46.81 MTD, matches the forwarded $59.99 receipt). Current Aug29–Sep28 cycle: $0 so far (fresh cycle) — real protection is the $40 Spend Management cap, which auto-pauses production if hit     |
| Sentry         | Free Developer                                                | Drops events past monthly quota, cannot bill                                                                                                                                                                                                                                                              | 78 accepted errors/24h (8% of warn) — OK, $0                                                                                                                                                                                                                      |
| PostHog        | Free                                                          | Free monthly event allowance, no card on file                                                                                                                                                                                                                                                             | 1,397 MTD events (0% of warn) — OK, $0                                                                                                                                                                                                                            |
| GitHub Actions | Free (org)                                                    | 2000 minutes/mo included; public repo bills $0 on standard runners regardless of minutes used                                                                                                                                                                                                             | 33,554 min MTD, net spend $0.00 — OK                                                                                                                                                                                                                              |
| Gmail API      | n/a (quota)                                                   | 15K units/min/user · 1.2M units/min/project (informational)                                                                                                                                                                                                                                               | $0                                                                                                                                                                                                                                                                |
| Paddle         | Sandbox                                                       | Revenue vendor (~5% + $0.50/txn as merchant-of-record); no spend meter                                                                                                                                                                                                                                    | $0 until revenue — webhook delivery OK                                                                                                                                                                                                                            |
| Razorpay       | Test mode                                                     | Revenue vendor (~2-3%/txn, India); subscriptions need Razorpay activation                                                                                                                                                                                                                                 | $0 until revenue — webhook delivery OK                                                                                                                                                                                                                            |
...
**Corrected 2026-08-29:** this subsection described `ANTHROPIC_ADMIN_KEY`
and a `cost_report` check as if they already existed. They didn't —
`check-vendor-limits.mjs` had no `checkAnthropic()` and no Anthropic row
in its `VENDORS` list; the daily watchdog has never once looked at
Anthropic spend. The check is now implemented (2026-08-29) matching what
was already documented below; it stays `UNCONFIGURED` until the founder
actually creates the key per step (a). Note also: the Admin API's own
docs say it is "unavailable for individual accounts" — if this Anthropic
account is individual-tier rather than an org/Console account, the check
will report `ERROR` with that exact message once the key is wired, which
...
live (see `prod-infra-bootstrap.md` Step 1). A budget never stops
spend, so **GCP has no hard cap in effect today**. Precisely what is
and is not shipped:

- **Shipped (code):** `infra/billing-hard-cap/index.js` — a Cloud
  Function that reads the budget's Pub/Sub message and unlinks the
  project from its billing account once cost crosses `KILL_RATIO`.
- **Not done (founder console work):** every step in
  `infra/billing-hard-cap/README.md` §Setup — create the
  `billing-budget-alerts` topic, create the $60 budget wired to it,
  create the `billing-hard-cap` runtime SA, grant it
  `roles/billing.projectManager` on the billing account, deploy with
  `DRY_RUN=true`, then flip it off. Tracked as the Open
  FOUNDER-FOLLOWUPS entry "Hard $60/mo billing cap on
  declutrmail-ai-prod" (2026-06-08).

Until that setup runs, the compensating controls are
...
2. ```bash
   ...
   ```
   **Corrected 2026-08-29:** this step previously said "wire both
   secrets when Pro lands," implying they weren't. **They already are**
   — the daily watchdog's own run history proves it: `VERCEL_TOKEN`/
   `VERCEL_TEAM_ID` appear in the job env of every run back to at least
   2026-08-23, and the check has returned real `$` figures each time.
   Re-run this step only if rotating the token.

**(b) Built-in cap** — **confirmed live 2026-08-29** (founder
screenshot, Dashboard → Settings → Billing): On-Demand Budget $40,
Notifications: On, Pause Projects: On, Pause Production Deployments:
On. An earlier draft of this section wrongly said this was "not yet
turned on" — that was asserted without ever having a way to check the
Vercel dashboard (the watchdog script has no API call that reads
Spend Management config, only spend itself), and was wrong. Nothing
left to do here. Caveat, per Vercel's own docs: checks run every few
minutes so pausing is not instantaneous, and unpausing after a pause
is manual per-project.
...
| **GCP**            | Billing → Reports, group by service (Cloud Run vs KMS vs egress)                           | Kill-switch is written but NOT armed — `infra/billing-hard-cap/` ships the Cloud Function that unlinks billing on breach, and its one-time console setup (Pub/Sub topic, $60 budget, runtime SA + `roles/billing.projectManager`, deploy) is still Open in FOUNDER-FOLLOWUPS ("Hard $60/mo billing cap", 2026-06-08). Until it is deployed, treat GCP as uncapped: scale the offending service down (`gcloud run services update <svc> --min-instances=0`), and if spend is truly runaway, Billing → Account management → disable billing on the project (nuclear — takes everything down) |
