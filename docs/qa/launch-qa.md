...
| sender-detail     | 2026-09-02 | first-timer · scared · heavy · editor (`usability-editor`)                                                        | yes       | 18 new / 0 inherited (first sender-detail run); 3 candidates REFUTED before filing (see Refuted below). 2 P1 also in `FINDINGS.md`; all 18 in `docs/qa/qa-worklist.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | _(Backfilled 2026-09-03 — this row was missing from the summary table; the full run record already existed as `### sender-detail — 2026-09-02` below and is unchanged.)_ Own worktree stack (api :4010/web :3010 — :4000/:3000 held by a different live worktree). Walked Sender Detail for 3 shapes (active financial sender, 6-year-dormant single-message sender, fresh single-message sender); real Archive+Undo and Protect+Unprotect round-trips, DB+Gmail-MCP verified; cross-tenant sender id → correct 404 isolation. `U` never pressed. Full detail below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| sender-detail     | 2026-09-03 | first-timer · scared · heavy (spot-check re-verification; editor pass not re-run — no new copy to review)         | no        | 0 new / 0 inherited — this run's mandate was re-checking the 18 existing rows live, not a fresh cold walk; all 18 flipped `Fixed 2026-09-03`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | **Live re-check of PR #717's 18 fixes**, plus 3 narrower follow-ons (#718, #719, #720) merged the same morning. Own worktree stack rebuilt from scratch (this worktree had no `.env.local`/`node_modules`/`.dev-db-identity` — first use since creation); found and killed THREE orphaned processes (api :4000, web :3000, worker health :8080) from a worktree (`ct-qa-archive-60c875`) already deleted from `git worktree list` — a new instance of the "orphan worker survives" class, ~22–25h uptime, gutted directory confirmed dead before killing. Verified each of the 18 rows against the live dev stack (D206 dev login, chintan.a.thakkar@gmail.com): dormant-sender hero copy (`gary@shipcobalt.com`, 9y-old last mail) now reads the editor's prescribed "Nothing in the last 12 months..."; privacy header now reads "subject and Gmail's preview snippet only"; partial-current-month framing correctly reads "so far in Sep" (after ruling out a `/senders/[id]` cold-compile race — 15.2s first-hit Next dev compile produced a stale intermediate render that self-corrected, not a product bug); read-rate now carries its denominator; Open-in-Gmail `aria-label`/`title` now match; Decision Timeline reads plain English with no raw `op <uuid>`; Protect button now says "Protected" with the exact reason inline (not title-only); disabled Unsubscribe now carries a `title` reason; the generic error state (forced via a garbage sender id) no longer repeats its title and now has both "Try again" and "Back to Senders"; `DESTRUCTIVE ACTIONS PREVIEW FIRST` is gone, replaced by "Nothing moves until you confirm". Spot-checked the 3 follow-ons via source: #719's `StatsStrip` deletion is clean (one deliberate historical comment, no live refs); #720's `displayId` wiring is correctly scoped to `activity-screen.tsx` (not sender-detail — confirmed out of this job's surface). Two informational, non-blocking observations recorded on the worklist's `## sender-detail` section rather than filed as new rows (both reasoned through, neither is a live regression): pre-fix `triage_decisions.reasoning` rows persisted before 2026-08-31 still render stale phrasing until re-scored (already named out-of-scope by `-01`'s own sibling note); PR #718's fixed cadence string is unreachable on the true `isPhone` (≤480px) collapsed sender-list row (`!isMobile` gate predates the fix and hides the whole evidence line there) — nothing false renders, but the commit message's "mobile row" framing overclaims the surface. Mobile-viewport tap-to-expand could not be driven this run (`computer` click timed out repeatedly with "Browser pane is currently hidden" against a `div[role=button]`; synthetic `MouseEvent`/pointer-event dispatch via `javascript_tool` did not trigger React's handler either) — recorded as inconclusive tooling, not a product finding, consistent with the same class of browser-pane mobile-emulation artifact logged by `archive`/`delete`/`mailbox-switch`/`senders-filtering`. No mutations performed (copy/UI re-check only); Outstanding restores stayed empty throughout. `U` never pressed. |
| triage            | 2026-09-03 | first-timer · scared (spot-check re-verification against a prior source-level prep pass; heavy/editor not re-run) | no        | 1 new / 0 inherited (`QA-sender-detail-20260903-01`, filed under sender-detail, discovered while cross-navigating from this job's own stack) — this run's mandate was re-checking the 19 existing merged-but-unconfirmed rows live                                                                                                                                                                                                                                                                                                                                                                                                            | **Re-check of triage's 9 rows still sitting at `Static-verified (WEAK)`**, dispatched a parallel read-only prep agent first (source cross-check against every row's Cause citation + git log, no browser/DB) to target the live pass efficiently. Machine had rebooted since the prior (sender-detail) session — redis container gone, api/worker/web all down, cleanly rebuilt with zero collisions this time (no orphans). Live-drove: queue order stability (2 fresh reloads, identical first sender and full 12-sender order — `-01`); zero `/api/triage/queue-size` hits in the api log across 2 loads, only `/api/triage/bootstrap` (`-05`, also reconfirms `20260828-03`'s single-query claim); expanded a row, all 4 stat tiles explicitly labeled including "PER MONTH 90D AVG" (`-10`, desktop only — 375px not retested); Today-strip past-tense copy and "marked read" vocabulary consistency both visually confirmed on the live queue (`-03`/`-07`/`-08`/`-12`, these were already 🟢 from 2026-09-01, reconfirmed incidentally). `-09` (undo-deadline timezone) and `20260828-01` (Temu day-bucket) confirmed via the SAME shared component/function exercised live by the `undo` job's real mutations this session, not independently re-driven from Triage's own UI. **Not live-tested this pass, still 🔵**: `-04` (Tier 1 billing cap — needs a real-Postgres-driver test, out of scope for a dev pass), `-06` (empty-state copy — needs `decidedToday=0`), `-11` (375px sticky footer), `20260828-02` (cross-surface 90-day-window comparison). `U` never pressed. Full detail: see the `qa-worklist.md` `## triage` section, updated in place (no separate `###` block — this was a targeted re-check of specific rows, not a fresh persona walk).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| undo              | 2026-09-03 | scared (real mutation round-trips; not a fresh persona walk)                                                      | no        | 0 new / 0 inherited — targeted re-check of the 2 rows still sitting at `Static-verified`-strength evidence (`-01`, `-04`); `-02`/`-03` correctly left untouched (need different repro conditions)                                                                                                                                                                                                                                                                                                                                                                                                                                             | **Two real mutation round-trips, both DB- and Gmail-MCP-verified.** Archived `sshanavaz@newtglobal.com` (1 msg, untouched single-message sender) then Undo'd it via the receipt banner — `activity_log` shows `reverted_at` set to the second; `/activity`'s "This window (30 days)" ARCHIVED tile read exactly **1**, matching a direct DB count of `action='archive' AND reverted_at IS NULL` rows in the trailing 30 days (1 of 25) — the freshly-undone cycle correctly excluded (`-01`). Deleted `utdalert_go@utdalert.utdallas.edu` (1 msg, "Inbox + archived" reach, since the message wasn't currently in Inbox) then Undo'd it — every surface (banner, toast, decision-record chip, undo-tray) read "Deleted to Gmail Trash" verbatim in the reader's local zone, consistently; Gmail MCP `search_threads` confirmed the message is out of `in:trash` and back in normal search results post-undo, a real provider-side round-trip, not just a DB flag (`-04`). Both rows flipped from `Static-verified` framing to a stronger, DB/Gmail-cross-checked `Fixed 2026-09-03`. `-02` (375px pre-hydration grid flash) needs a throttled/pre-hydration capture this session's tooling can't reliably produce — left untouched. `-03` correctly confirmed still `Open` (no code change, definitional gap). `U` never pressed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| archive           | 2026-09-03 | scared + first-timer (targeted re-check + one new live discovery)                                                 | yes       | 1 new / 0 inherited (`QA-archive-20260903-01`) — Screener's `decide-preview.tsx` never received `-02`'s staleness-indicator fix, live-confirmed as a real, reachable gap (not just a source-read theory as the prior session left it)                                                                                                                                                                                                                                                                                                                                                                                                         | Live-drove: expanded a Triage row, confirmed all 4 stat tiles labeled incl. "PER MONTH 90D AVG" (`-01`); opened Screener, expanded "Priyanka Singh" (116mo old, `sends 0/mo`), clicked Archive — the confirm dialog's "Why we suggested this" sentence rendered bare, with zero staleness/age indicator, unlike Triage's identical dialog which has had one since PR #670/#712 — cancelled before confirming (0 matching emails, sender has nothing in inbox right now) (`-02`'s sibling, new row `-20260903-01`); `/activity` loaded with 20 manually-actioned rows, every one read "by you", zero "VIA MANUAL" instances (`-06`). Corrected `-05`'s status text, which had gone stale twice over: the "Destructive actions preview first" string it shipped was superseded by PR #712 then rewritten again by the unrelated sender-detail PR #717 — live text today reads "Nothing moves until you confirm" everywhere; the row's underlying 🔴 reason (3-way D226 modal-HEADER grammar mismatch, tracked at `20260901-01`) is untouched by that rename and stays accurately Open. `-03`/`-04` not independently re-driven this pass (source-confirmed only, no red flags from a dedicated prep agent's read). `U` never pressed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| onboarding        | 2026-09-03 | scared (single high-severity forced-state repro; not a fresh persona walk)                                        | yes       | 0 new / 0 inherited — this run's sole mandate was the dedicated live pass `QA-onboarding-20260828-01` (P0) had been waiting on since 2026-09-01                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | **The P0 live pass.** Captured the real primary mailbox's (`chintan.a.thakkar@gmail.com`) exact `provider_sync_state` row into `docs/qa/launch-qa.md`'s Outstanding restores BEFORE mutating (ready/ready/100/NULL); forced `readiness_status='syncing', current_stage='building_sender_index', progress_pct=42`; reloaded `/senders` — rendered "Still syncing… 7,988 senders in this mailbox for chintan.a.thakkar@gmail.com — this scan hasn't finished, so the list may be incomplete or stale, and may change once it's done," never a false "Synced through" claim or "No active senders." Restored the captured exact values immediately after, re-queried to confirm, cleared the Outstanding-restores row same-session — reloaded once more post-restore and confirmed the normal "Results as of Sep 3, 2026, 3:06 PM PDT" copy returned. Did not additionally force the `readiness_status='failed'` variant or the zero-sender empty-state branch (would need destructively clearing `senders` rows for this mailbox — out of scope for a reversible pass). The other 4 onboarding rows (`-02`,`-03`,`-04`,`-05`, all non-Tier-1) were not re-driven this session — a source-level prep pass found their fix code present and matching with no red flags, left at their existing status. `U` never pressed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| delete            | 2026-09-03 | scared + first-timer (targeted re-check + one new live discovery)                                                 | yes       | 1 new / 0 inherited (`QA-delete-20260903-01`) — Screener's own Delete confirm dialog never got `-08`'s zero-match header fix, live-confirmed, same shape as the same-day `QA-archive-20260903-01`                                                                                                                                                                                                                                                                                                                                                                                                                                             | Live-drove: Senders "More actions" → Delete (Wells Fargo, `notify.wellsfargo.com`) — confirm modal title read "Move email from 1 sender to Gmail Trash" (`-02`); the same sender's Delete preview's mail-location sentence read "452 emails in your inbox · 273 emails elsewhere in Gmail (archived or under a label)" (`-09`). Expanded a Screener row (Priyanka Singh, 0 in inbox) and clicked Delete — dialog read "Delete Priyanka Singh's inbox email / 0 matching emails," an active-move header directly above a zero-count body, exactly `-08`'s bug, unfixed on Screener (new row filed). Also reconciled `-04`'s status, which the worklist had at "Approved, NOT fixed, needs a founder call on hydration-gating risk" since 2026-08-30 — that note predates and is unaware of PR #671 (merged 2026-08-29, the day before), which already removed the UTC override as part of the `triage`/`undo` fix; live-confirmed via this session's own `undo`-job mutations (no console hydration warnings on either cycle) — flipped to Fixed, noted the stale reasoning rather than silently overwriting it. `-03`'s repro needs forcing `subscriptions.billing_cycle` on the dev workspace's row — checked first and that row is the REAL live Paddle sandbox subscription the `billing` job's own history flags as never-to-be-touched; declined to mutate it even reversibly, left source-confirmed only. `-05` reconfirmed via the cross-job evidence of this session's own tray-undo round-trips (same mechanism, different row). `U` never pressed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| sign-in           | 2026-09-03 | first-timer (all no-auth marketing/pre-consent copy; no session needed for 6 of 7)                                | no        | 0 new / 0 inherited — targeted re-check of the 7 merged-not-confirmed rows                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **6 of 7 rows live-confirmed, zero mutations, zero auth needed** (D134's session-blind marketing pages plus pre-consent `/sign-in` copy). `/` FAQ "Can it mess up my inbox?" reads the unhedged `ACTION_SAFETY_SUMMARY` verbatim, no "exactly" claim (`-01`) — noted in passing that the hero H1 still says "see exactly what moves," a known/parked precision-overclaim left alone by design (D250 locks the H1). `/` and `/sign-in` step 1 both read the corrected `OAUTH_SCOPE_DISCLOSURE` verbatim (`-02`). `/` hero disclaimer states the 50-cleanup-action Free cap (`-05`). The "What Google will ask you to allow" `<details>` is present and expands to reveal the `/sign-in` link — collapsed by default rather than always-visible, a shape change from the original plan worth a founder glance but functionally reachable (`-06`). `/sign-in` step 2 promises no scan duration (`-07`). `/sign-in?auth_result=inbox_limit` renders the tier-agnostic, jargon-free alert (`-09`). **`-03` deliberately not driven**: its repro needs forcing the dev workspace's `subscriptions.billing_cycle`, and that row is confirmed to be the real live Paddle sandbox subscription CLAUDE.md flags as never-to-be-touched — left at merged-not-confirmed, source-evidence only. `-04`/`-10` reconfirmed still genuinely `Open` (zero source hits for either id anywhere in the repo). `U` never pressed — not reachable pre-auth anyway.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| triage/archive    | 2026-09-03 | scared (targeted follow-up smoking pass, same-day)                                                                | no        | 0 new / 0 inherited — closed out 4 rows the earlier passes left `🔵`/blocked                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | **Second smoking pass, same session, in response to "focus on remaining to be green."** Opened Triage's Archive ActionSheet (10xTravel) once and read 3 rows off the one dialog: sticky footer visible at true 375px with no scroll (`QA-triage-20260827-11`, previously blocked by a flaky browser-pane retry — retried clean this time); "Skip this dialog for Archive — the preview shows in the row instead. Change this in Settings." checkbox (`QA-archive-20260828-04`). Cross-referenced the Today strip's "last 90 days" against a row's "90d" tile on the same page load (`QA-triage-20260828-02`). Found a genuine 0-match window live (`alerts@f5bot.com`, all 68 inbox messages <25 days old) and selected "1 year+" — notice read "...none are older than the 1 year+ window", the window LABEL not a day count (`QA-delete-20260829-03`). All 4 flipped to `Fixed 2026-09-03`. Did not attempt `QA-triage-20260827-06` (needs the queue genuinely empty — would require 12 real Gmail mutations to reach `decidedToday=0` naturally, declined) or the two rows already confirmed blocked on a real, protected Paddle sandbox row (`QA-triage-20260827-04`'s prod-driver gap is separate — dev PGlite only, structural). `U` never pressed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| activity          | 2026-09-18 | first-timer · scared · heavy (both mailboxes) · editor (`usability-editor`)                                       | no        | 10 new / 1 inherited (QA-undo-20260828-03 re-confirmed). 9 candidates went to `finding-refuter`: 6 refuted or downgraded, 3 survived. 2 P1 → `FINDINGS.md`; all 10 in `docs/qa/qa-worklist.md` `## activity`                                                                                                                                                                                                                                                                                                                                                                                                                                  | **Could not break the core job.** Undo from Activity on a 21-day-old Archive: `activity_log.reverted_at` set, `LabelActionWorker` succeeded `affectedCount 5`, counters moved 1→0 / 20→19 live. Replaying the same undo token returned the original `revertedAt` and ran no second job (one `LabelActionWorker` line in the log — the probe would have shown two). Unknown token 404, missing CSRF 403, bad cursor 400, no session 401. Mailbox switch mid-search with Grouped on: zero rows from the old mailbox survived, sidebar counts re-scoped (probe would have shown Temu rows under the second mailbox). 375px usable. **Preflight found the orphan-stack class again:** :3000/:4000 were a 15-day-old stack from worktree `zen-wu-258d39`, and its Sep-3 worker survived `dev-up.sh --stop`, held :8080, and crashed this checkout's worker with `EADDRINUSE` — killed by pid after confirming cwd; all three listeners re-verified against this checkout before the walk. `dev-up.sh` also fails in a worktree on the shared Redis container name unless `COMPOSE_PROJECT_NAME=declutrmail`. **The sweep out-earned the seed:** the refuted "Recovered stays 0" candidate led `defect-class-sweeper` to three live siblings, measured via `assert-dev-db.sh --exec` — 24 senders whose Sender Detail history shows an undone action as standing, a Triage Today strip missing the `reverted_at IS NULL` filter every sibling reader got in #671 (37 same-day undos ever), and the weekly value-receipt email windowing "actions you undid" on the original action's date. **Not driven:** `U` (Safety); "Try this action again" in the recovery dialog (would archive 85 real emails for no QA value); Export support bundle (file download). Kill-the-worker, two-tab and 429 probes skipped — covered by the 2026-08-28 / 09-03 `undo` runs on the same code path. **State left behind:** no forced values, Outstanding restores untouched and empty. One product action persists — the undone Temu archive put 5 promotional emails back in the founder's inbox.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| protect           | 2026-09-19 | first-timer · scared · heavy · editor (`usability-editor`)                                                        | no        | 9 new / 1 inherited (QA-senders-20260901-08 re-confirmed, and its fix is the cause of new row -02). 9 candidates to `finding-refuter`: 5 refuted or downgraded. 2 P1 → `FINDINGS.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                          | **The safety promise held under every attack.** UI: 25 Protected senders selected → Archive/Unsubscribe/Later/Delete all "0 of 25" and disabled. Server: `POST /api/actions/preview/bulk` with one Protected + one unprotected sender returned totals for the unprotected one only and `protectedCount: 1`; a bulk Archive of two Protected senders returned 409 `NO_ACTIONABLE_SENDERS` both without and WITH `override:true`; a single-sender Archive without override returned `PROTECTED_SENDER`; `PATCH /senders/:id/policy` on the other mailbox's sender returned `SENDER_NOT_FOUND`. Zero `action_jobs` rows were created by any probe (queried) — each probe would have shown a job row. The automatic reason "you starred a message" was checked against real Gmail via the Gmail MCP: one `STARRED` message dated 2025-11-10, inside the one-year window. Double-click on Protect wrote one `marked_protected` row, not two. **Sticky Unprotect: read, not driven.** Unprotect left `(is_protected=false, protection_reason='starred')`, the memory pin the sweep's `WHERE is_protected=false AND protection_reason IS NULL` cannot match. The "Sync now" probe proved nothing — `recordsProcessed 0` means an empty sender scope, which is an explicit no-op; the nightly unscoped sweep was not run. **Forced state:** one `sender_policies` row pinned by id (70c09fbb…), restore written to Outstanding restores BEFORE the toggle, run, verified by re-query (`t / starred / NULL / keep`), my two toggle rows deleted (0 left), Outstanding row cleared. **Not covered by the restore:** a trigger re-stamped `updated_at` to 2026-09-19 (was 2026-08-10), and `ScoreWorker` re-scored the sender while it was unprotected (`decisionsWritten 1`, `produced_at` refreshed). **Measured by the sweep:** 44,938 of 129,346 stored snippets (35%) contain an HTML entity; 0 truncated mid-entity; 0 RFC 2047-encoded subjects or display names, so that half of the sweeper's class was dropped. **Not driven:** `U` (Safety); confirming an Archive on a Protected sender (would move 181 real emails); Autopilot exclusion (no active rules — `rulesEvaluated 0`). Stack run without `dev-up.sh` (Docker daemon hung): `dev-api.sh` + `dev-worker.sh` directly against the already-running Redis, all PIDs confirmed from this checkout.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
Not a fresh surface walk — this run implemented the 4 findings the founder
approved out of the 2026-09-03 decision batch (`QA-archive-20260901-01`,
`QA-archive-20260903-01`, `QA-delete-20260903-01`,
`QA-sender-detail-20260903-01`), then ran the diff through Codex adversarial
review as it grew, fixing what came back each round rather than reporting and
moving on.

**Implemented:** unified the D226 preview-eyebrow header grammar across
Triage's single/batch action sheets and the Senders confirm modal (new
`packages/shared/src/copy/preview-eyebrow.ts`); added the staleness label and
the zero-match "Nothing to move" header to Screener's `decide-preview.tsx`,
mirroring Triage's identical preview; fixed the Sender Detail 404 race
(`detail`/`messages`/`timeseries`/`history` resolve as four independent,
unordered queries against the same `CurrentMailboxGuard`-scoped id — checking
only `detail` missed a 404 that a sibling settled first).

**Review found 10 more defects across 4 rounds, all fixed and
negative-controlled** (revert → confirm the accompanying test fails with the
exact wrong behavior → restore → confirm green) — see PR #723's body for the
full per-round breakdown. Shape of what recurred: a live count computed once
and trusted everywhere else that needed it (bulk sender counts, quota math,
confirm gates) instead of re-derived per consumer; a generation-boundary
timestamp compared against the wrong side of an async gap (`invalidateQueries`
awaiting before the boundary was stamped, then a narrower in-flight-fetch
version of the same shape after that was fixed); and a confirm gate that knew
"not Protected" but not "actually executable for this verb" (bulk Unsubscribe
against a mailto-only remainder).

**One finding investigated and refuted, not fixed:** round 3 flagged a
mechanism where an in-flight request could return the previous mailbox's data
after a switch. Traced `CurrentMailboxGuard` →
`MailboxAccountsService.resolveActiveForRequest`: it re-reads
`users.preferences->>'activeMailboxId'` live from Postgres on every request,
no cache, no session claim, no client-sent mailbox parameter on these routes —
so a request whose guard evaluation happens after the switch commits resolves
against the new mailbox regardless of client-side timing. Round 4 independently
confirmed this reading before sharpening the finding to the real, narrower
in-flight-first-fetch case (fixed).

**Live-smoked** against the real dev stack (D206 dev login): Screener's
zero-match title at both inbox-only and all-mail reach (no title/notice
contradiction in either), Triage's unified single-sender eyebrow and
true-zero header, Senders' bulk Unsubscribe eyebrow and confirm-enables-
once-preview-resolves. Zero console errors. Not independently hand-triggered
live: the batch-sheet zero-actionable/stale-refetch gate (no domain batch
card in this account's current data) and the sender-detail 404 race itself
(needs a specific cross-mailbox stale condition) — both covered by
deterministic, negative-controlled automated tests instead.

```
New: 0   Inherited and still open: 0   Refuted: 1 (investigated, documented, not filed)
QA-archive-20260901-01 · P3 · Fixed, PR #723
QA-archive-20260903-01 · P2 · Fixed, PR #723
QA-delete-20260903-01 · P3 · Fixed, PR #723
QA-sender-detail-20260903-01 · P2 · Fixed, PR #723
```
...
| 2026-09-18 | activity | "Your last 7 days … Recovered 0 / No outcomes" is false right after a successful Undo | 6 already known (QA-undo-20260828-03), 2 by design (`reverted_at IS NOT NULL → null`, activity.read-service.ts:1276), 3 the undone row's `occurred_at` was 21 days outside the window regardless | REFUTED — inherited row re-confirmed instead |
| 2026-09-18 | activity | "6 NEED ATTENTION" contradicts "Nothing needs your attention" on one screen | 2 both panels carry their own scope label; 3 all 6 failures are older than 7 days so both sentences are true; 6 recorded under QA-undo-20260828-01 "What this is NOT — do not re-file" | REFUTED — residue (count is unlinked text) filed as P3 |
| 2026-09-18 | activity | Stat tiles disagree with the rows under them ("ARCHIVED 0 / 0" over two Archived rows; "DELETED 0" over 5 Deleted rows) | 3 every cited row was `Undone`, one of them by this run's own undo; 2 the exclusion is the shipped fix for QA-undo-20260828-01 (#671) | REFUTED as filed — the unit gap re-filed on clean evidence (19 standing archives = 1,335 emails) |
| 2026-09-18 | activity | Duplicate history rows prove protect/archive double-writes | 3 behno rows are two real actions (distinct `undo_token` + `action_job_id`, 2 min apart); 4 the only `marked_protected` writer is behind a state diff; 1 NerdWallet's orphan rows are `sender-policy.spec.ts` e2e residue on the shared dev DB | REFUTED |
| 2026-09-18 | activity | An Activity row cannot be opened to see which emails were affected, and the Gmail link shows a different set | 2 by design — the ratified claim is a preview-time sample (`ACTION_PREVIEW_CLAIM`), and QA-sign-in-20260829-01 deleted "exactly which emails" as an overclaim; the link's name and title say "Open <sender> in Gmail" | REFUTED — backlog idea only: `buildActionScopeSearchLink` exists with zero callers |
| 2026-09-18 | activity | Failed rows lose their timestamp at desktop width | 1 the photographed state (desktop grid inside an 800px pane) needs the JS and CSS breakpoints to disagree — the `innerWidth: 0` pane signature | DOWNGRADED, not filed — re-test at a true 901–1100px viewport, where the refuter's arithmetic says the last grid track may clip |
| 2026-09-19 | protect | One unconfirmed click on the Protected badge removes the only safety state, with no undo and no disclosure that it is permanent | 2 every element is a written decision — toggle-is-the-undo (`senders-policy.service.ts:33-35`), the sticky pin is §2.6's own mandate; 3 a toast DOES fire (`sender-detail-page.tsx:1169`; my 4s wait missed it) and the stickiness IS disclosed on Settings → Protected senders; 6 pre-specified in `FINDINGS.md:1996-2000` | REFUTED — residue (the caveat is missing from the Sender Detail badge) folded into -09 |
| 2026-09-19 | protect | After Unprotect the highlighted button becomes Unsubscribe while the suggestion says Keep | 4 no read-rate heuristic exists: `deriveDefaultPrimary` is protected→keep, unsubReady→unsubscribe, >180d→archive, else keep, and protection was masking rule 2; 6 the "— highlighted button is Unsubscribe" text is the shipped fix for QA-sender-detail-20260902-07; Unsubscribe still goes through the D226 preview | REFUTED — residue (highlight reason is `title`-only) folded into -09 |
| 2026-09-19 | protect | The "protected 511" chip returns 25 senders, so chip counts lie | 2 documented design: chips are mailbox-wide per-axis absolutes "so the user can predict the next click" (`compose-strip.tsx` header, `senders.read-service.ts:1029`); the 2026-09-01 senders-filtering run drove the same 523→498→25 sequence and understood it | REFUTED |
| 2026-09-19 | protect | The AI-written "why" text is editorial inference in the third person with a stale number | 2 third person is the prompt's own instruction; "11 monthly" is exactly 33/3 over the same 90-day window; "outweighs" is literally true (the star is a Phase-A short-circuit); AI labelling is fallback-only by design | DOWNGRADED — the one unbacked clause ("being a temporary lapse") survives as -06: the reasoning path has no output validator |
| 2026-09-19 | protect | Automatic protection leaves no Activity record, so the user never learns when or why | 2 Activity's stated scope is actions by you, Autopilot and your rules — a nightly sweep is none; 3 the WHY renders on Sender Detail, and `protection_set_at` is written for 509 of 511; only 1 orphan "Unprotected" row exists | DOWNGRADED to P3 and folded into -09: `protection_set_at` is on the wire and rendered nowhere |
