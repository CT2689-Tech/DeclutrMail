...
| 🟡  | QA-sync-20260831-03 | P1  | The app-shell header (`SyncNowButton`) and the banner whose job is "your sync is broken" (`SyncErrorBanner`) both render nothing for `readiness='failed'` — the banner keys on a different signal (`last_sync_error_at`) that an initial-sync failure never stamps                                                                  | **Review complete, 2 rounds.** Round 1 fixed `AuthExpiredError` reconnect recognition, the `SyncErrorBanner` co-render conflict, and the silent-vanish-after-retry gap (`0b675112`). Round 2 confirmed all three hold and found one non-blocking overclaim in the retry-success toast ("Scan started" claimed an unconfirmed BullMQ enqueue — the durable row moving to `queued` is all the response proves) — reworded to "Scan queued" in `7da14596`. Cite `2925b5e5`, `0b675112`, `7da14596`.                                                                                                                                                                                                                                                          |     |
