...
**As of 2026-09-03: 🟢 12 confirmed · 🔵 4 merged-not-yet-live-confirmed · ⬜ 3 open (by design, no code change).**
This line is a hand-count and goes stale the moment a row moves — it carries
a date so you can see that, rather than trusting a number nothing regenerates.
A run that moves a row updates it. Remaining 🔵 rows and why: `-04` (Tier 1
billing cap — still needs a real-Postgres-driver test, dev PGlite only so
far), `-06` (empty-state copy — needs `decidedToday=0`, not forced this
session), `-11` (sticky preview footer at 375px — not driven this session),
`20260828-02` (shared 90-day-window cross-surface comparison — not directly
compared side-by-side this session).

|     | id                    | sev                       | one line                                                                                                                                                        | status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | PR   |
| --- | --------------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 🟢  | QA-triage-20260827-01 | P1                        | The daily queue's `ORDER BY` has no tiebreak, so _which_ 12 senders appear is undefined and any write reshuffles the list under the reader                      | Fixed 2026-09-03 — live-verified: loaded `/triage` twice (fresh navigation each time), first sender ("Bank of America") and full 12-sender order identical across both loads. Live tie fixture confirmed present in DB (107 rows share the exact `unsubscribe/0.87` verdict/confidence pair) without a stability demonstration this would be the exact reshuffle scenario the fix targets.                                                                                                                                                               | #663 |
...
| 🟢  | QA-triage-20260827-05 | P2                        | D30's adaptive 5–12 queue size is dead code — no client ever calls `queue-size`, so everyone gets the hard max 12                                               | Fixed 2026-09-03 — closed the prior session's ambiguity with a source read: `apps/api/src/triage/triage.controller.ts`'s `bootstrap()` calls `this.triage.getQueueSize(mailbox.id)` directly server-side (not a separate client round-trip), confirmed zero web callers of `/queue-size` or the raw `/queue` endpoint anywhere outside comments/stories. Live-confirmed the api log shows only `/api/triage/bootstrap` hits (2/2 page loads) and zero `/api/triage/queue-size`/`/api/triage/queue` hits — matches "correctly inlined", not "still dead". | #671 |
...
| 🟢  | QA-triage-20260827-09 | P2                        | The undo deadline renders in UTC in the toast and in the reader's zone in the preview, two clicks apart                                                         | Fixed 2026-09-03 — the real archive+undo mutation this row was waiting on ran, via the `undo` job's own re-check this session (same shared `formatExpiry`/`undo-tray.tsx` component Triage uses): toast, receipt banner, and decision-record chip all read the identical reader-local deadline ("3:00 PM PDT" and, on a second cycle, "3:04 PM PDT") with zero UTC/local mismatch. Not independently re-driven from Triage's own UI this pass, but the fixed code path is shared and mutation-tested.                                                    | #671 |
| 🟢  | QA-triage-20260827-10 | P2                        | Two stat tiles are windowed and two are not, with nothing saying so; at 375px "90D" orphans onto its own line                                                   | Fixed 2026-09-03 — live-verified at desktop width: expanded a Triage row (Capital One Shopping), all 4 stat tiles explicitly labeled ("PER MONTH 90D AVG", "MARKED READ 90D", "LAST SEEN", "RECEIVED ALL TIME"). **375px orphan-wrap specifically not re-tested this pass** (browser-pane mobile-viewport tooling was unreliable earlier this session — see the sender-detail mobile-tap notes) — desktop label-presence confirmed, narrow-width wrap behavior still relies on the 2026-09-01 test-only evidence.                                        | #671 |
| 🟢  | QA-triage-20260827-11 | P2                        | The preview's footer — reversibility line, Cancel, confirm — sits below the fold on a 375px phone                                                               | Fixed 2026-09-03 — live-verified at true 375px: opened the Archive ActionSheet for 10xTravel, resized to mobile, footer ("Reversible for the 30-day undo window from Activity." + Cancel + Archive) stayed pinned to the bottom of the viewport, fully visible with no scroll needed.                                                                                                                                                                                                                                                                    | #671 |
...
| 🟢  | QA-triage-20260828-02 | P3                        | "The last 90 days" is implemented independently in 4+ places with no shared definition; nothing makes them agree                                                | Fixed 2026-09-03 — live side-by-side comparison on the same `/triage` page load: Today strip reads "...email you received in the last 90 days", row tile reads "None marked read in 90d" — same window, consistent phrasing, no drift.                                                                                                                                                                                                                                                                                                                   | #663 |
| 🟢  | QA-triage-20260828-01 | P2                        | "LAST SEEN today" is shown for mail that arrived yesterday — the label buckets by elapsed hours, not calendar day                                               | Fixed 2026-09-03 — not re-driven against the exact named Temu example this pass, but the shared `daysSince` function (same one this row's fix touches) was live-observed producing correct calendar-day buckets across many senders this session on Sender Detail and Triage: "9y ago", "yesterday", "today" (for genuinely-today mail), "2d ago" — all calendar-correct, no elapsed-hours artifacts seen anywhere.                                                                                                                                      | #663 |
...
| 🔴  | QA-undo-20260828-03 | P3  | The "Recovered" outcome tile can never register a user's own Undo (a different mechanism entirely — retried-after-failure jobs) and nothing in the product defines it     | Open · re-confirmed 2026-09-18 (`/ct-qa activity`: a fresh Undo left the tile at 0; label still undefined on screen)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | · approved 2026-09-19; relabelled "Fixed on retry" in PR #742 (at review cap, with QA-activity-20260918-05…-10) |
...
|     | id                     | sev | one line                                                                                                                                                                                                                                                                                                                                   | status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | PR   |
| --- | ---------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 🟢  | QA-archive-20260828-01 | P2  | The Triage volume tile shows a 90-day-derived average unlabelled as such, while the adjacent read-rate tile on the same row does label its window                                                                                                                                                                                          | Fixed 2026-09-03 — live-verified: expanded a Triage row (Capital One Shopping), all 4 stat tiles explicitly labeled — "PER MONTH 90D AVG", "MARKED READ 90D", "LAST SEEN", "RECEIVED ALL TIME"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | #670 |
| 🟢  | QA-archive-20260828-02 | P2  | The D226 action-preview dialog — the one screen a destructive mutation cannot skip — renders frozen LLM rationale text with no staleness indicator, unlike every other place the same text renders                                                                                                                                         | Fixed 2026-09-01 for the ORIGINAL filed scope (Triage's D226 preview) — live-verified at merge (`b369f4ab`, Victoria's Secret Panty Party). **Live re-driven 2026-09-03**: opened Screener's own decide/confirm dialog (Priyanka Singh, Archive) and confirmed the sibling gap the detail block below already named — Screener's `decide-preview.tsx` still renders the bare rationale with zero age indicator, unlike Triage. See new row `QA-archive-20260903-01` — this row stays Fixed for its literal filed scope (Triage), the sibling is tracked separately.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | #670 |
...
| 🟢  | QA-archive-20260828-04 | P3  | "Show this in the row next time" describes where the preview renders, not that it skips the dialog, and not that the choice is per-verb                                                                                                                                                                                                    | Fixed 2026-09-03 — live-verified: opened the Archive ActionSheet (10xTravel), checkbox reads "Skip this dialog for Archive — the preview shows in the row instead. Change this in Settings." Names the verb, names skipping the dialog, names where to change it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | #670 |
| 🔴  | QA-archive-20260828-05 | P3  | The toolbar's pre-selection hint (fixed by this row) vs. the deeper D226 modal-header grammar (split out as `QA-archive-20260901-01`, see below)                                                                                                                                                                                           | At review cap 2026-09-01 — 2 substantive Codex rounds, founder ship/no-ship call needed. **Round 1:** retexted toolbar hint to "Nothing changes until you preview" — found (a) false for Keep (applies immediately, no preview, D40) and (b) confirmed the real 3-way inconsistency this row was originally filed against is the PREVIEW MODAL header grammar, not the toolbar hint — split out below as its own row rather than silently closed here. Responded: retexted to "Destructive actions preview first" (true for all 5 verbs), strengthened the test. **Round 2:** found the strengthened test still used a loose `toContain` (would pass with misleading text appended after the same opening words) and Sender Detail's own toolbar had zero test coverage of this hint at all — a Sender-Detail-only reversion would've stayed green. Responded: both test files now assert the exact `>text</span>` node boundary, not a substring; Sender Detail toolbar test added from scratch. Negative control re-run on both files: reverted → RED → restored → GREEN (80/80). **Not sent for round 3 — at cap.** These are mechanical-shaped test-assertion tightenings responding to exactly what round 2 named, but per the Rules a rendered-string/test-assertion change is substantive, and round 3 would exceed the 2-round limit. Founder call: ship as-is (fixes match round 2's stated failure modes) or take one more look. **Status text stale as of 2026-09-03**: the "Destructive actions preview first" string this row landed has since been superseded again — PR #712 shipped it, then the unrelated sender-detail run (PR #717, `QA-sender-detail-20260902-18`) rewrote the same shared string a third time. Live-confirmed today: the toolbar hint now reads **"Nothing moves until you confirm"** everywhere (Triage and Sender Detail), not "Destructive actions preview first". The underlying reason this row is still 🔴 — the deeper 3-way D226 modal-HEADER grammar mismatch — is untouched and unaffected by that later rename; see `QA-archive-20260901-01` below, still accurately Open. Not independently re-driven this session (only the single-sender Archive/Delete preview headers were seen live today, both matching the row's description of surface-specific wording — the batch-preview and Senders-confirm-modal comparison itself is source-confirmed only via this session's prep read, unchanged since 2026-09-01). |      |
| 🟢  | QA-archive-20260901-01 | P3  | The D226 preview HEADER grammar differs by surface, not just the toolbar hint: Triage's real preview modal renders `Preview · {verb}` (e.g. "Preview · Archive"), its batch modal renders `Preview · {verb} · multiple senders`, and the Senders confirm modal renders the generic `Preview · before anything changes` with no verb at all | **In review 2026-09-03.** Founder decision: unify to one shared header pattern. Fixed via a new shared helper `packages/shared/src/copy/preview-eyebrow.ts` (`previewEyebrowLabel(verb, senderCount)`) wired into all 3 surfaces — Triage single sheet now `Preview · Archive`, Triage batch sheet now names the REAL eligible count (`Preview · Archive · 3 senders`, not "multiple"), Senders confirm modal now names the verb and count too (was fully generic). Unit tests added at all 4 sites (the helper + 3 consumers), negative-controlled (reverted, watched 3 tests go RED with the exact expected old text, restored, GREEN). Live-smoked all 3 surfaces. Hit and worked around a real Next.js `optimizePackageImports` runtime bug importing the new export via the `@declutrmail/shared/copy` barrel (typecheck+vitest clean, broke only in the Next dev server, survived a full restart) — added a direct subpath export instead. Diff sent to Codex for adversarial review 2026-09-03. **Review passed 2026-09-03** after 4 Codex adversarial rounds (round 1: 4 findings fixed; round 2: 4 findings fixed; round 3: 2 findings fixed, 1 refuted with source evidence — see PR body; round 4: 2 findings fixed, clean otherwise). PR #723.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | #723 |
| 🟢  | QA-archive-20260828-06 | P3  | Activity's per-row source label uses the internal enum voice "VIA MANUAL" instead of "by you"                                                                                                                                                                                                                                              | Fixed 2026-09-03 — live-verified on `/activity`: 20 manually-actioned rows loaded, every one reads "by you", zero instances of "VIA MANUAL" found                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | #670 |
| 🟢  | QA-archive-20260903-01 | P2  | Screener's own decide/confirm dialog (`decide-preview.tsx`) never got QA-archive-20260828-02's staleness-indicator fix — it renders the bare frozen rationale with no age label, live-confirmed, while Triage's identical dialog now shows one                                                                                             | **In review 2026-09-03.** Founder decision: fix now, mirroring the proven Triage pattern. Added `scoredAgeLabel`/`useNow()` age label next to the frozen rationale in `decide-preview.tsx`, matching `action-preview-presentation.tsx`'s existing implementation exactly. Unit tests added (age-label-present and age-label-absent cases), negative-controlled (reverted, watched both new tests go RED, restored, GREEN). Live-smoked: Screener → Priyanka Singh → Archive dialog now reads "Last checked 2 days ago" beside "Why we suggested this". Diff sent to Codex for adversarial review 2026-09-03, same review batch as `QA-delete-20260903-01` (same file) and `QA-archive-20260901-01`. **Review passed 2026-09-03** after 4 Codex adversarial rounds (round 1: 4 findings fixed; round 2: 4 findings fixed; round 3: 2 findings fixed, 1 refuted with source evidence — see PR body; round 4: 2 findings fixed, clean otherwise). PR #723.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | #723 |
...
|     | id                        | sev | one line                                                                                                                                                                                                                                                                                                                       | status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | PR   |
| --- | ------------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 🟢  | QA-onboarding-20260828-01 | P0  | `/senders` can assert something false about the user's own inbox during an active sync — it either denies any senders exist, or shows a stale pre-disconnect snapshot labelled with a "Synced through" time it never measured — reachable on ANY ordinary returning login mid-resync, not only reconnect                       | **Fixed 2026-09-03 — the dedicated live pass this row was waiting on, done.** Captured the real primary mailbox's exact `provider_sync_state` row (ready/ready/100/NULL) into `docs/qa/launch-qa.md`'s Outstanding restores BEFORE mutating; forced `readiness_status='syncing', current_stage='building_sender_index', progress_pct=42`; reloaded `/senders` live — rendered "**Still syncing…** 7,988 senders in this mailbox for chintan.a.thakkar@gmail.com — this scan hasn't finished, so the list may be incomplete or stale, and may change once it's done." No false "Synced through"/"Results as of" claim, no "No active senders" — exactly the fix the regression test asserts. Restored the exact captured pre-mutation values immediately after, re-queried to confirm (`ready`/`ready`/100/NULL), cleared the Outstanding-restores row. Reloaded once more post-restore: correctly back to "Results as of Sep 3, 2026, 3:06 PM PDT · 7,988 senders in this mailbox". Did not additionally force the `readiness_status='failed'` variant or the zero-sender-rows empty-state branch this pass (would need destructively clearing `senders` rows) — the `syncing` variant was the highest-value repro and is now live-confirmed. | #673 |
...
| 🟢  | QA-delete-20260829-02 | P2  | The Senders Delete confirm modal's title ("Delete email from 1 sender") never says Trash, unlike Triage's equivalent header                                                                                                                               | Fixed 2026-09-03 — live-verified: Senders → "More actions" → Delete (Wells Fargo, `notify.wellsfargo.com`) opened the confirm modal, title read "Move email from 1 sender to Gmail Trash" exactly. Cancelled, no mutation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | #674 |
| 🟢  | QA-delete-20260829-03 | P2  | Empty-window notice states an age in days ("not older than 6 days") beside a control labelled in months ("6 months+")                                                                                                                                     | Fixed 2026-09-03 — genuine 0-match window hit live: found `alerts@f5bot.com` (68 inbox messages, all <25 days old), opened Delete, selected "1 year+" (0 matches). Notice read "68 emails from this sender are in your inbox, but none are older than the **1 year+ window**. Widen the window to include them." — names the window label, not a day count. (A secondary line separately shows "0 emails currently match (older than 365 days)" — a correct day-equivalent for context, not the unit-confused "6 days for a 6-months+ window" bug this row was filed for.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | #674 |
| 🟢  | QA-delete-20260829-04 | P2  | Post-delete Undo deadline renders in UTC while every other timestamp on the same surfaces renders in the viewer's local clock                                                                                                                             | **Fixed 2026-09-03 — reconciling conflicting history.** The 2026-08-30 note below argued this was deliberate (hydration-safety) and recommended a `useNow()`-gating design before touching it, unaware that PR #671 (`4f8b35a1`, merged 2026-08-29 07:08 UTC — the SAME day this row was filed, one day before the 08-30 note) had already removed `undo-tray.tsx`'s `timeZone: 'UTC'` override as part of fixing the identical bug on `triage`/`undo` (`QA-triage-20260827-09`/`QA-undo-20260828-04`). Current `formatExpiry` (source-confirmed today) has no `timeZone` override at all and carries a comment citing those exact QA ids. Live-confirmed twice this session with real mutations (Archive+Undo on `sshanavaz@newtglobal.com`, Delete+Undo on `utdalert_go@utdalert.utdallas.edu`): the toast, receipt banner, decision-record chip, and global Undo tray all read the identical reader-local deadline ("3:00 PM PDT"/"3:04 PM PDT") with zero console hydration warnings on either cycle — the exact mismatch the 08-30 note worried a naive fix would cause did not occur. Treat the note below as superseded by the same-week PR, not as a still-open design question. | #671 |
| 🟢  | QA-delete-20260829-05 | P3  | Senders receipt strip doesn't observe an undo performed via the global Undo tray and keeps asserting stale "Moved to Gmail Trash" state (all verbs, not Delete-specific)                                                                                  | Fixed 2026-09-03 — not independently re-driven with the exact "concurrent second tab" shape this row names, but the same mechanism (a page's receipt strip reacting to a global-tray-initiated revert) was exercised twice this session on sender-detail itself: Archive+Undo and Delete+Undo, both via the same floating Undo-tray control, both times the page's own toolbar/decision-timeline correctly cleared with no stale "Archived"/"Deleted to Gmail Trash" residue and no duplicate toast.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | #674 |
...
| 🟢  | QA-delete-20260829-08 | P3  | A zero-match Delete preview's header still reads as an active move ("Move inbox email from X to Gmail Trash" above a "0 matching emails" body)                                                                                                            | Fixed 2026-09-03 for the filed scope (Triage) — not independently re-driven with a genuine 0-match Triage sender this session (all queued senders had nonzero inbox matches), but live-drove and CONFIRMED the identical unfixed bug on **Screener** — same shape as `QA-archive-20260903-01`: Screener's `decide-preview.tsx` never got this fix. Live-confirmed: Screener → Priyanka Singh (0 in inbox) → Delete → dialog read "Delete Priyanka Singh's inbox email / 0 matching emails" — an active-move header directly above a zero-count body, exactly the bug this row fixed on Triage. Cancelled, no mutation. **Third live-confirmed instance of "Screener doesn't inherit Triage's D226-preview fixes"** (after `QA-archive-20260828-02`'s staleness indicator) — worth a dedicated `/ct-class` sweep rather than three separate one-off sibling rows. New row filed below: `QA-delete-20260903-01`.                                                                                                                                                                                                                                                                           | #674 |
| 🟢  | QA-delete-20260903-01 | P3  | Screener's own Delete confirm dialog (`decide-preview.tsx`) never got `QA-delete-20260829-08`'s zero-match header fix — it reads as an active move ("Delete X's inbox email") directly above a "0 matching emails" body, unlike Triage's identical dialog | **In review 2026-09-03.** Founder decision: fix now, mirroring the proven Triage pattern (same PR as `QA-archive-20260903-01` — both land in the same `decide-preview.tsx` file). Hoisted the existing `moves` check earlier in the component and added the `moves && liveCount === 0` branch to the title, matching `action-preview-presentation.tsx`'s "Nothing to move from X right now" exactly. Unit tests added (all 3 mail-moving verbs zero-match, Keep/Unsubscribe untouched, nonzero-count unaffected), negative-controlled (reverted, watched 4 new tests go RED, restored, GREEN — all pre-existing tests, including the window-reconciliation ones, stayed green throughout). Live-smoked: Screener → Priyanka Singh (0 in inbox) → Delete dialog now reads "Nothing to move from Priyanka Singh right now". Diff sent to Codex for adversarial review 2026-09-03. **Review passed 2026-09-03** after 4 Codex adversarial rounds (round 1: 4 findings fixed; round 2: 4 findings fixed; round 3: 2 findings fixed, 1 refuted with source evidence — see PR body; round 4: 2 findings fixed, clean otherwise). PR #723.                                                      | #723 |
| 🟢  | QA-delete-20260829-09 | P3  | Mail-location sentence mixes a singular subject ("this email") with a four-digit population and drops the unit noun on one side of the split                                                                                                              | Fixed 2026-09-03 — live-verified: opened Wells Fargo's (`alerts@notify.wellsfargo.com`) sender-detail Delete preview, sentence read exactly "Where this sender's mail is now: 452 emails in your inbox · 273 emails elsewhere in Gmail (archived or under a label)." — plural subject, unit noun on every segment. Cancelled, no mutation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | #674 |
...
| --- | ---------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 🟢  | QA-sign-in-20260829-01 | P1  | Homepage trust-strip copy claimed exact precision ("you see exactly which emails are affected"), contradicting the product's own `ACTION_PREVIEW_CLAIM` two lines below it in the same file | Fixed 2026-09-03 — live-verified: `/` FAQ "Can it mess up my inbox?" now reads "Before a manual action moves email, you see the count and what changes in Gmail..." verbatim from `ACTION_SAFETY_SUMMARY`, no "exactly" claim. **Known, out-of-scope gap (not a regression, D250-locked):** the hero H1 still reads "...and see exactly what moves." — same precision-overclaim class, explicitly parked by this row's own plan (D250 locks the H1), not listed in the doc's parked-inventory though — worth a founder glance if the lock is ever revisited.                                                                                           |
| 🟢  | QA-sign-in-20260829-02 | P1  | Pre-consent disclosure undersold the `gmail.modify` grant as "organize your Gmail" when the scope covers send/compose too                                                                   | Fixed 2026-09-03 — live-verified on both consumers: `/` (collapsed "What Google will ask you to allow") and `/sign-in` step 1 both read "Google's screen asks for one Gmail permission, gmail.modify, and words it more broadly than DeclutrMail uses it: DeclutrMail moves and labels email and never sends email as you." verbatim, matching the single-source `OAUTH_SCOPE_DISCLOSURE` constant.                                                                                                                                                                                                                                                    |
| 🔵  | QA-sign-in-20260829-03 | P2  | Pricing tier CTA / `?plan=X` deep link auto-expanded the plan-change confirm panel without checking the visitor's `currentTier`                                                             | **Deliberately not live-driven this session.** The repro needs forcing `subscriptions.billing_cycle` on the dev workspace's subscription row — checked first via `assert-dev-db.sh`, and that row IS the real, live Paddle sandbox subscription (`sub_01kys14ejmvjg6fe2f1jza8kkd`, tier=pro/active/annual) the billing job's own history already flags as never-to-be-seeded-or-deleted. Declined to mutate even reversibly. Source-confirmed only (prep agent read): `plan-picker.tsx:282-301`'s guard (`intentPlan !== currentTier \|\| intentCycleDiffers`) matches the fix description exactly, with tests for both the blocked and allowed cases. |
...
| 🟢  | QA-sign-in-20260829-05 | P2  | Homepage disclaimer omitted the 50 cleanup-action/month Free-tier cap at the exact point a visitor decides if Free fits                                                                     | Fixed 2026-09-03 — live-verified: `/` hero disclaimer reads "Free · no card · 50 cleanup actions a month · 30-day undo on Archive, Later and Delete" — cap present, sourced from `TIER_MANIFEST.free.cleanupActionsPerMonth` (=50).                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 🟢  | QA-sign-in-20260829-06 | P3  | `/sign-in`'s pre-consent explanation page was unreachable from any nav or homepage link                                                                                                     | Fixed 2026-09-03 — live-verified, with a shape-change worth knowing: the shipped fix wraps the link inside a `<details>/<summary>` ("What Google will ask you to allow"), collapsed by default, rather than the plan's literal always-visible line. Expanding it shows "See what DeclutrMail can and can't access →" linking to `/sign-in` — satisfies "reachable", but the link isn't visible without a click. Accepting this as fixed (link exists and works); flag to the founder only if "reachable" was meant to imply "visible at a glance".                                                                                                     |
| 🟢  | QA-sign-in-20260829-07 | P3  | `/sign-in` step 2 promised a scan duration ("a few minutes") the sync gate deliberately never states elsewhere                                                                              | Fixed 2026-09-03 — live-verified: `/sign-in` step 2 reads "The first scan runs on its own — we email you when your inbox is ready." No duration claim anywhere.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 🟢  | QA-sign-in-20260829-09 | P3  | `/sign-in`'s `inbox_limit` alert was a 3-instruction run-on sentence leaking internal terms ("workspace", "inbox slot")                                                                     | Fixed 2026-09-03 — live-verified: `/sign-in?auth_result=inbox_limit` alert reads "Every Gmail connection your plan allows is already in use. Sign in with any connected Gmail to disconnect it, or upgrade to connect more." — tier-agnostic, no "workspace"/"inbox slot" jargon, "Compare plans →" links to `/pricing`.                                                                                                                                                                                                                                                                                                                               |
...
| 🟢  | QA-sync-20260831-06 | P1  | Reconnecting ANY previously-synced mailbox unconditionally nulls its history cursor and forces a full resync — bypassing both the cheap incremental-resume path and the codebase's own existing `cursorTooOld` escalation ladder, which already handles the genuinely-stale-cursor case                                             | Fixed 2026-09-25 — PR #775: a sign-in or connect of an active, synced mailbox with a healthy grant keeps its cursor and enqueues one incremental catch-up. Still a full scan, as the founder scoped it: a disconnected mailbox, a revoked grant, a queued/running/failed initial sync, a ready row with no cursor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |     |
...
**All 18 originally-filed rows closed `Fixed 2026-09-03`; 1 new row (`-19`,
dated `20260903`) filed the same day from a live discovery during a separate
re-check session.** PR #717 merged, plus 3 narrower
follow-ons (#718 mobile `/mo` cadence label, #719 dead `StatsStrip` removal,
#720 `displayId` wiring — the latter two outside this worklist's scope but
spot-checked in the same run). A 2026-09-03 re-check run drove the live dev
stack (D206 dev login) and confirmed all 18 symptoms gone — see the
2026-09-03 sender-detail ledger entry for per-item evidence. Two informational
observations surfaced during the re-check, neither risen to a new row (reasons
in the ledger entry): (a) `sender_policies`/`triage_decisions.reasoning` rows
scored before the fix (2026-08-31) still render pre-fix phrasing until a
sender is re-scored — already named out-of-scope by `-01`'s own sibling note;
(b) PR #718's fixed cadence string is unreachable on the true `isPhone`
(≤480px) collapsed list row (pre-existing `!isMobile` gate hides the whole
evidence line there) — not a live regression since nothing false renders, but
worth a founder FYI that the commit's "mobile row" framing overclaims which
surface it touches.
...
|     | id                           | sev | one line                                                                                                                                                                                                                                                                                                                                                                                    | status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --- | ---------------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🟢  | QA-sender-detail-20260902-01 | P1  | The hero sentence says "Hasn't mailed you yet." for a sender that mailed the user 6 years ago — same screen's Relationship stat and message list contradict it                                                                                                                                                                                                                              | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack): `gary@shipcobalt.com` (last mail 9y ago, `totalReceived=1`) hero now reads "Nothing in the last 12 months. Their last email was 9y ago." — matches the editor's prescribed copy exactly. Read-rate sibling confirmed too: "no email in the last 90 days." See 2026-09-03 sender-detail ledger entry for full evidence (screenshots). Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                               |
| 🟢  | QA-sender-detail-20260902-02 | P1  | "We never render bodies" / "We never store the body" / "Exports never contain message bodies" all sit beside or govern real Gmail-snippet body text — the export claim is on a persisted, shareable artifact                                                                                                                                                                                | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-03 | P2  | Volume KPI/hero renders a partial or stale month in the same unit as complete months ("Sent 3 in Sep.", 2 days into September) — reads as a full month's cadence                                                                                                                                                                                                                            | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-04 | P2  | "0% of the last 90 days' email was marked read" has no denominator — 0% of 2 emails and 0% of 200 are different facts, same sentence                                                                                                                                                                                                                                                        | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-05 | P3  | "Open all in Gmail" overclaims — the link is a `from:` search, which excludes Spam and Trash; aria-label and title describe the same control two different ways                                                                                                                                                                                                                             | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-06 | P2  | Decision timeline vocabulary disagrees with the Activity feed for the identical `activity_log` row, one click apart via "View in Activity →"; today's Unsubscribe row also reads ungrammatically ("You Unsubscribe requested")                                                                                                                                                              | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-07 | P2  | Two unsourced "what should I do" signals on one screen — the toolbar's fact-derived primary verb (Archive) and the "Optional suggestion" banner (Later) can disagree with no explanation of which is which                                                                                                                                                                                  | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-08 | P3  | "scored yesterday" is internal scoring-engine vocabulary leaking into user-facing copy on Sender Detail, Triage and Screener via one shared string                                                                                                                                                                                                                                          | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-09 | P3  | The generic load-error state repeats its own title as the first sentence of its body                                                                                                                                                                                                                                                                                                        | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-10 | P3  | Decision timeline renders a raw `op <uuid>` on every row; at 375px it's the widest thing in the column and wraps                                                                                                                                                                                                                                                                            | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-11 | P3  | Recent Messages stacks three near-duplicate chrome lines (eyebrow + heading + trust line) above the list; the heading count is just how many rows loaded, so it changes as you scroll                                                                                                                                                                                                       | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-12 | P3  | The suggestion banner's "This suggestion does not change email" is the third statement of that same fact on one screen                                                                                                                                                                                                                                                                      | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-13 | P3  | Relationship KPI restates one number in a second unit ("13 yr" / "159 months") instead of the one fact the reader can't compute (the start date)                                                                                                                                                                                                                                            | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-14 | P2  | Delete has no visual distinction from Keep in the action toolbar — the only verb that puts mail in Gmail Trash renders identically to the safest verb                                                                                                                                                                                                                                       | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-15 | P2  | Protect's exact reason (D245: "show the exact reason") is title-only, which never opens on touch; the button's own label doesn't say "Protected" — evidenced by 4 Protect/Unprotect toggles inside 2 days on the same sender in this run's own Decision Timeline                                                                                                                            | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-16 | P2  | Disabled Unsubscribe gives no reason it's disabled, though the screen already knows why (no List-Unsubscribe channel)                                                                                                                                                                                                                                                                       | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-17 | P2  | The generic load-error state's only action is "Try again" with no escape hatch; the sibling `NotFoundState` for the same route already has "Back to Senders"                                                                                                                                                                                                                                | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260902-18 | P3  | "DESTRUCTIVE ACTIONS PREVIEW FIRST" is uppercase D226 mechanism-language ("destructive", "preview") that reads as a noun phrase on first pass, shared with Triage via one literal                                                                                                                                                                                                           | **Fixed 2026-09-03.** PR #717 merged 2026-09-03. Live re-check (D206 dev login, running dev stack) confirmed the described symptom is gone — see 2026-09-03 sender-detail ledger entry for the specific evidence (screenshots, page text, source cross-checks) on this row. Prior history: 2 substantive Codex rounds ran pre-merge (round 1: a broken template.ts zero-volume check that broke the existing F009 workers regression test, a reverse-lookup map drift tsc couldn't catch, an imprecise "over 6 months" claim — all fixed; round 2: a cross-mailbox stale-data regression in the new error guard, a rounding-before-zero-check ordering bug in template.ts, a missing 4th branch in the toolbar re-derivation, an unverified UTC test — all fixed and negative-controlled). Founder authorized merge directly ("Codex limits were reset. Proceed. If it looks good then merge it.") rather than a 3rd round, per the skill's 2-round cap. Diff final (29 files, incl. a 3-instance sibling sweep of round 2's own stale-`YYYY-MM-DD` finding), typecheck+lint clean, 2646 tests pass across the affected suites incl. per-fix negative controls, live-smoked pre-merge (fresh tab, D206 dev login, eBay Feedback sender).                                                                                                                                                                                                                                                                                                                                                            |
| 🟢  | QA-sender-detail-20260903-01 | P2  | The same sender-id 404 (a real, existing sender belonging to a DIFFERENT mailbox than the active one) nondeterministically renders one of two different error surfaces — the purpose-built "Sender not found · Back to Senders" NotFoundState, or the generic "We couldn't load this sender / Try again" exception-boundary fallback — depending on timing, not on the underlying condition | **In review 2026-09-03.** Founder decision: fix now. Root-caused precisely: the guard at `sender-detail-page.tsx` checked ONLY `detail.error` for a 404, while `messages`/`timeseries`/`history` are three independent, unordered queries hitting the identical `CurrentMailboxGuard` for the same sender id — whichever settles its 404 first (not always `detail`) decided which branch rendered. Fix: check all four queries for the 404 (`[detail, messages, timeseries, history].some(q => q.error instanceof ApiError && q.error.status === 404)`) before falling into the generic-error branch, making the outcome order-independent. Added a deterministic regression test that forces the exact bad ordering via a manually-controlled pending promise on `detail`'s response while siblings 404 immediately — negative-controlled (reverted, the new test failed with the literal wrong UI text — "We couldn't load this sender" — in the failure output, restored, GREEN; all 35 pre-existing tests in the file stayed green throughout). Live-smoked: 2 fresh navigations to the same cross-mailbox sender id, both now consistently render "Sender not found". Diff sent to Codex for adversarial review 2026-09-03, including the open question of whether the four endpoints' 404 semantics are truly symmetric on the API side. **Review passed 2026-09-03** after 4 Codex adversarial rounds (round 1: 4 findings fixed; round 2: 4 findings fixed; round 3: 2 findings fixed, 1 refuted with source evidence — see PR body; round 4: 2 findings fixed, clean otherwise). PR #723. | #723 |

### QA-sender-detail-20260902-01 — "Hasn't mailed you yet." on a sender with real history

**Status: Fixed 2026-09-03** (PR #717, live re-check confirmed — see worklist
table row above for the re-check evidence). Detail below is the original
2026-09-02 filing, kept as the record of what was wrong.
...
|     | id                      | sev | one line                                                                                                                                                                                               | status                                                                                                                                                                                                                                                                                                                                                      |
| --- | ----------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🔴  | QA-activity-20260918-01 | P1  | **Tier 1 — privacy.** Sender domains are sent to Brandfetch in production, and no public page or the Gmail-data registry names it; ADR-0034 still says production must leave the key unbound           | Approved by the founder 2026-09-19. Decision: disclose, keep the tier. At review cap — PR #741, privacy-auditor FAILED three rounds, every time on a sentence the author wrote; 205b0706 applies round 3's accepted wording verbatim. Waiting on a founder yes to the final public wording. Brandfetch licensing is a separate open FOUNDER-FOLLOWUPS entry |
| 🔵  | QA-activity-20260918-02 | P1  | Sender Detail's decision history shows an action the user already undid as still standing, while Activity marks the same record Undone                                                                 | Approved by the founder 2026-09-19. Merged #740 — architecture-guardian PASS; independent adversarial review (Codex CLI out of date) round 1 found 2, round 2 CLEAN against fd2b391b. Not `Fixed`: no later run has re-checked the symptom                                                                                                                  |
| 🔵  | QA-activity-20260918-03 | P2  | Triage's Today strip keeps counting a decision after the user undoes it — the one `activity_log` reader that missed #671's `reverted_at IS NULL` filter                                                | Approved by the founder 2026-09-19. Merged #740 — same review trail as -02                                                                                                                                                                                                                                                                                  |
| 🔵  | QA-activity-20260918-04 | P2  | **Needs a founder call.** The weekly value-receipt email counts "actions you undid" by when the original action happened, so a week spent undoing older actions reports zero and can suppress the send | Approved by the founder 2026-09-19. Founder delegated the call: `undone` is windowed on when the user undid it, and an undo-only week stays silent (D189). Merged #740                                                                                                                                                                                      |
| 🔴  | QA-activity-20260918-05 | P2  | The five stat tiles never say they count actions rather than emails, and they ignore the source and action chips directly beneath them                                                                 | Approved by the founder 2026-09-19. At review cap — PR #742, design-system gate + adversarial review: round 1 FAIL (4 blocking), round 2 confirmed 3 fixed and 1 open, fixed in 1f7cc698 with the gate's prescribed approach. Goes to the founder rather than a third lap                                                                                   |
| 🔴  | QA-activity-20260918-06 | P2  | "Protected" and "Skipped" each mean two different things in the same column (editor-filed)                                                                                                             | Approved by the founder 2026-09-19. At review cap — PR #742, design-system gate + adversarial review: round 1 FAIL (4 blocking), round 2 confirmed 3 fixed and 1 open, fixed in 1f7cc698 with the gate's prescribed approach. Goes to the founder rather than a third lap                                                                                   |
| 🔴  | QA-activity-20260918-07 | P2  | The recovery dialog's title is machine-assembled into broken English ("Review failed archived") and its result grid uses worker vocabulary ("reconciled", "verified set")                              | Approved by the founder 2026-09-19. At review cap — PR #742, design-system gate + adversarial review: round 1 FAIL (4 blocking), round 2 confirmed 3 fixed and 1 open, fixed in 1f7cc698 with the gate's prescribed approach. Goes to the founder rather than a third lap                                                                                   |
| 🔴  | QA-activity-20260918-08 | P3  | "6 need attention" is plain text, and no visible control reaches those rows outside the 7-day card; the failed-unsubscribe cause lives only in a hover tooltip                                         | Approved by the founder 2026-09-19. At review cap — PR #742, design-system gate + adversarial review: round 1 FAIL (4 blocking), round 2 confirmed 3 fixed and 1 open, fixed in 1f7cc698 with the gate's prescribed approach. Goes to the founder rather than a third lap                                                                                   |
| 🔴  | QA-activity-20260918-09 | P3  | Every row with no Undo renders an empty bordered pill segment beside the Gmail link                                                                                                                    | Approved by the founder 2026-09-19. At review cap — PR #742, design-system gate + adversarial review: round 1 FAIL (4 blocking), round 2 confirmed 3 fixed and 1 open, fixed in 1f7cc698 with the gate's prescribed approach. Goes to the founder rather than a third lap                                                                                   |
| 🔴  | QA-activity-20260918-10 | P3  | Copy bundle (editor-filed): "Followups" vs "Follow-ups" in three spellings, "UNSUBSC / RIBES" at ≤375px, an empty state that names an unlabelled control and ships no button, a verbose banner         | Approved by the founder 2026-09-19. At review cap — PR #742, design-system gate + adversarial review: round 1 FAIL (4 blocking), round 2 confirmed 3 fixed and 1 open, fixed in 1f7cc698 with the gate's prescribed approach. Goes to the founder rather than a third lap                                                                                   |
...
First filed 2026-09-19 (`/ct-qa protect`). 9 new survivors; 1 inherited row lives under `## senders`
(QA-senders-20260901-08, re-confirmed there). 5 candidates were refuted or downgraded before
filing — see the ledger. **The safety promise itself held under every probe**; what survived is
what the product SAYS about Protected, plus one data-quality class the run tripped over.

|     | id                     | sev | one line                                                                                                                                                                                                     | status |
| --- | ---------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ |
| ⬜  | QA-protect-20260919-01 | P2  | **Backfill half is Tier 1 (prod data).** A third of all stored Gmail preview snippets contain raw HTML entities (`Here&#39;s`), shown on Sender Detail, written into the data export, and sent to Anthropic  | Open   |
| ⬜  | QA-protect-20260919-02 | P2  | With only Protected senders selected, the desktop bulk bar's "unprotect to include" note collapses to one word per line and pushes Delete off the edge — caused by the fix for QA-senders-20260901-08        | Open   |
| ⬜  | QA-protect-20260919-03 | P2  | On desktop the Protected state is a 10px dot with a hover tooltip on the grid card, an unlabelled shield in the table, and nothing at all in the quick view; the phone layout shows the word                 | Open   |
| ⬜  | QA-protect-20260919-04 | P1  | The confirm preview on a Protected sender never says the sender STAYS Protected after the action, so a user cleaning up a wrongly-protected sender leaves believing they are done (editor-filed)             | Open   |
| ⬜  | QA-protect-20260919-05 | P1  | Two automatic-protection reasons do not state the rule that ran: "you starred a message" omits "in the past year", and "Gmail marks it important" hides 3+ messages, in Primary, in a year (editor-filed)    | Open   |
| ⬜  | QA-protect-20260919-06 | P2  | The AI reasoning path has no output check: a suggestion was saved calling the user's 0% read rate "a temporary lapse", a prediction nothing in the model's input supports and its own prompt forbids         | Open   |
| ⬜  | QA-protect-20260919-07 | P2  | **Tier 1b — public claim.** `/security` and `/privacy` say auto-protection fires past "the documented threshold", and no public page documents it; the help glossary says Protected "locks a sender to Keep" | Open   |
| ⬜  | QA-protect-20260919-08 | P2  | Naming bundle (editor-filed): Unprotect has five names and two pending labels, the effect of Protected is phrased six ways, the Senders screen never defines it, and a story still says "VIPs"               | Open   |
| ⬜  | QA-protect-20260919-09 | P3  | Hover-only or missing context on Sender Detail: no "this is permanent" on the Unprotect badge, the highlighted-verb reason, when protection began, and a "checked today" age over weeks-old AI prose         | Open   |

### QA-protect-20260919-01 — raw HTML entities in a third of stored snippets

**Steps.** Open any sender whose recent mail has an apostrophe in its preview → "Recent messages" shows `Here&#39;s why it happened.`
**Evidence.** Measured via `assert-dev-db.sh --exec`: **44,938 of 129,346** `mail_messages.snippet` rows (35%) contain an entity; 0 are truncated mid-entity; 0 subjects or display names are RFC 2047-encoded (that half of the sweeper's class was dropped on the numbers).
**Cause.** Gmail returns `snippet` HTML-escaped; [gmail-client.service.ts:327](apps/api/src/gmail/gmail-client.service.ts:327) copies it through, and no decoder exists or is installed anywhere in the repo.
**Siblings (one stored value, three consumers).** Render — [recent-messages.tsx:250](apps/web/src/features/senders/detail/recent-messages.tsx:250). Data export, JSON and CSV — [export.service.ts:214](apps/api/src/account/export.service.ts:214), `:268`; an export is not re-rendered, so a bad one stays bad on the user's disk. The Brief prompt sent to Anthropic — [brief-llm-anthropic.adapter.ts:243](apps/api/src/adapters/brief-llm-anthropic.adapter.ts:243).
**The one correct fix** is a decode at ingest in `getMessageMetadata()`, BEFORE `initial-sync.worker.ts:1250`'s `.slice(0, 300)`; per-render decoding cannot help the export or the prompt. Decoding must produce text, never markup.
**Refuter's strongest objection.** Cosmetic — no fact is wrong; it would be P3 on the render alone and is P2 because the export is a persisted artifact.
**Founder call.** The ingest decode is an ordinary fix. Correcting the 44,938 stored rows is a production data migration (CLAUDE.md §9) and is NOT bundled into the same approval.
**Regression test.** Adapter spec: a `snippet` of `It&#39;s &amp; &lt;b&gt;` comes back as `It's & <b>` as TEXT; worker spec that the 300-cap is applied after decoding.

### QA-protect-20260919-02 — the all-Protected bulk bar overflows

[selection-bar.tsx:230-247](apps/web/src/features/senders/selection-bar.tsx:230) is `display:flex` with no `flexWrap` and a fixed `height: 52`; the `protectedLockNote` span (`:107-114`) has no `flexShrink: 0`, so it shrinks to one word per line, and the five buttons (`:153-170`) have none either, so Delete clips last. The refuter ruled out the pane artifact: `AppShell` is flex, not grid, and `useIsAtMost` reads `matchMedia`; an 800px pane leaves 722px for the row and a 1024×768 desktop leaves 726px. Reachable 481–1366px.
**Unmeasured:** widths are derived from source constants. Confirm with one real 1024×768 screenshot before fixing.

### QA-protect-20260919-03 — Protected is a tooltip on desktop

[sender-card.tsx:198-213](apps/web/src/features/senders/grid/sender-card.tsx:198) renders a 10px dot whose word and effect live only in `title` / `aria-label`; [sender-table.tsx:777-802](apps/web/src/features/senders/sender-table/sender-table.tsx:777) a shield with `aria-label="Protected"` and no reason; `grid/sender-peek.tsx` and `table/sender-row-detail.tsx` nothing at all. The un-swept siblings of QA-sender-detail-20260902-15 and QA-senders-20260901-08, both accepted here as defects.
**Corrected by the refuter:** the glyph after the sender's name is the quick-view affordance, not a Protected marker; and the overflow offering A/U/L/D on a Protected sender is by design (protection is about bulk and automatic actions).

### QA-protect-20260919-04 and -05 — what the product says at the moment of action (editor-filed, P1)

- **04** — [confirm-action-modal.tsx:1130-1131](apps/web/src/features/senders/confirm-action-modal.tsx:1130). Now: "This sender is Protected — it is normally kept out of bulk and automatic actions. This action applies to this sender only." Use: "This sender stays Protected after this action, so bulk and automatic cleanup will keep skipping it." Triage's `protected-notice.tsx:104-106` already says it; Senders and Sender Detail do not. "normally" is a hedge — it is the rule.
- **05** — [protection.ts:113](packages/shared/src/copy/protection.ts:113) → "you starred a message in the past year"; `:115` → "Gmail marked 3+ of their emails important in Primary this year". The sweep's rules are windowed (`automatic-protection.ts:145-147`, `:244-246`) and the nightly pass RETIRES a protection when the signal ages out, so without the window the user cannot predict the shield expiring.

### QA-protect-20260919-06 — the AI reasoning has no output check

`extractText` ([anthropic-haiku.adapter.ts:179-190](apps/api/src/adapters/anthropic-haiku.adapter.ts:179)) validates only `stop_reason`; the text is stored verbatim in `triage_decisions.reasoning`. The prompt's own rule (line 82) says "Never invent details not present in the input". **Refuter's objection:** one adjective in one row — a model-compliance miss with no guard, not a wrong product rule. Every number in the sentence checked out.

### QA-protect-20260919-07 — a public claim with nothing behind it (Tier 1b)

[security/page.tsx:130-132](<apps/web/src/app/(marketing)/security/page.tsx:130>) and `/privacy` `:188-190`: "cross the documented threshold". Use: "cross a fixed threshold — 3+ replies, a star in the past year, or 3+ Gmail-important emails in Primary in the past year." And [glossary-content.ts:45](apps/web/src/features/help/glossary-content.ts:45): "locks a sender to Keep" is false — a single-sender action is allowed and expected (`actions.service.ts:425-430`).

### QA-protect-20260919-08 and -09 — the editor's bundle

Exact replacement text for every string is in the editor's report. In short — **08:** one word for the act ("Unprotect" / "Unprotecting…" / "<name> is no longer Protected."); one exported `PROTECTION_EFFECT` sentence next to `protectionReasonClause` instead of six hand-rolled ones; define Protected in "How Senders works" (`senders-screen.tsx:2462`); the bulk note points at Settings → Protected senders, because that screen has no Unprotect; one bulk-409 sentence on API and FE; trim the Settings intro; the inbox simulator's seventh definition (`triage/data.ts:656`); `Protected` capitalised on the chip and the Triage pill; `'VIPs · receipts'` in `kpi-strip.stories.tsx:50`. **09:** the Sender Detail badge gains "automatic protection won't come back"; the highlight reason and `protection_set_at` become visible text; `score.worker` stops refreshing `produced_at` over reused prose; `generatedBy` reaches the Sender Detail wire.

**Already fixed by #742, so not filed:** the editor's "Protected means two things on Activity" — the tile now reads "Skipped for Protected".
**Found, not filed:** the `D38` citation on the chip-count design (`compose-strip.tsx`, `senders.read-service.ts:1029`) is the umbrella mis-tag CLAUDE.md §3 names — D38 is first-time education. Snippets also carry long runs of U+200C padding that the same render site shows untreated.
