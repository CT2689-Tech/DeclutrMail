...
|     | id                  | sev | one line                                                                                                                                                                                                                                                                                                                            | status                                                                                                                                                                                                                                                                                                                                                                                 | PR  |
| --- | ------------------- | --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| 🟡  | QA-sync-20260831-01 | P0  | Triage's empty state renders "Nothing needs a decision right now" — a confident positive claim — while the active mailbox's sync is `failed`, and Triage has zero sync awareness of any kind                                                                                                                                        | Review round 1 found the PARENT header still said "Nothing waiting." over the fixed child body — fixed in `0b675112`, negative-control verified. Awaiting round 2.                                                                                                                                                                                                                     |     |
| 🟡  | QA-sync-20260831-02 | P0  | Senders' `mailboxStillSyncing` readiness guard covers `queued`/`syncing` only; `failed` falls through to the exact "Synced through &lt;now&gt;" false-currency bug F032 was filed and fixed to kill — for the one state that fix didn't enumerate                                                                                   | Review round 1 found the guard only covered the default unfiltered view (search/filtered views still fell through) and the failed freshness copy overclaimed provenance of a partial index — both fixed in `0b675112`, negative-control verified. Awaiting round 2.                                                                                                                    |     |
| 🟡  | QA-sync-20260831-03 | P1  | The app-shell header (`SyncNowButton`) and the banner whose job is "your sync is broken" (`SyncErrorBanner`) both render nothing for `readiness='failed'` — the banner keys on a different signal (`last_sync_error_at`) that an initial-sync failure never stamps                                                                  | Review round 1 found 3 gaps: `AuthExpiredError` not recognized as needing reconnect (onboarding gate's own set now shared), `SyncErrorBanner` could co-render a doomed incremental retry beside the fix, and a successful retry left the button silently vanishing. All fixed in `0b675112`, negative-control verified. Awaiting round 2.                                              |     |
| 🟡  | QA-sync-20260831-04 | P0  | A non-active connected mailbox's persistent incremental-sync failure renders an affirmative **"Ready"** in Settings and no tag at all in the account menu — worse than silence                                                                                                                                                      | Review round 1 (non-blocking): tightened the error/success-tie edge case (`>` → `>=`) and added a negative-control test, in `0b675112`. Loading/error-state fallthrough to "Ready" and the dot-color mismatch are real but left open — see FOUNDER-FOLLOWUPS.                                                                                                                          |     |
| 🟡  | QA-sync-20260831-05 | P1  | No in-app signal ever fires for a background sync that fails (`useMailboxSyncToasts` only announces `→ready`), and `useMe`'s own poll is self-starving — it only refetches while a mailbox is already known to be syncing, so a `ready→failed` flip is never even fetched without a manual reload                                   | Review round 1 confirmed the `useMe` starvation gap is real but pre-existing/explicitly declined per this diff's own comment — no code change; cite commit `2925b5e5`.                                                                                                                                                                                                                 |     |
...
| 🟡  | QA-sync-20260831-07 | P1  | The onboarding `SyncFailed` screen's copy names the real cause for an auth failure ("Google revoked our access... Reconnect the account") but its only button re-queues a full scan with the SAME dead token — no reconnect action exists in that file at all                                                                       | Review round 1 found no code defect (verdict: "not a same-file gap") — only test-coverage gaps (AuthExpiredError click-wiring, failed+escape with an auth fixture), closed in `0b675112`. Cite `2925b5e5` + `0b675112`.                                                                                                                                                                |     |
...
| 🟡  | QA-sync-20260831-09 | P2  | The product calls this event "scan" in onboarding and in the retry endpoint's own semantics, "SYNC FAILED"/"Sync failed" in Settings and the account menu — and one Senders string literally matches `check-microcopy.sh`'s own banned "senders indexed" pattern, shipped anyway because the hook never sweeps existing files       | Review round 1 found `SYNC_NOT_READY.retryable` still `true` despite the endpoint 409ing on an identical retry — fixed in `0b675112`, negative-control verified. Awaiting round 2.                                                                                                                                                                                                     |     |
| 🟡  | QA-sync-20260831-10 | P2  | Five smaller copy/robustness defects on the same surface: a 409 message reused for 4 different states, a toast promising self-recovery that never happens, "Synced through" mislabelling a request-compute timestamp even when healthy, a silently-failing retry button, and a timeout toast pointing at a label hidden below 900px | Review round 1 (item 1, shared with QA-09): `SYNC_NOT_READY.retryable` fixed in `0b675112`. The unverified "ready-but-cursorless" 4th state Codex added to item 1's framing was checked against the only writer (`markReady`, which always persists a non-null cursor) and found unreproducible — comment reverted to the 3 confirmed states, not asserted as fixed. Awaiting round 2. |     |
