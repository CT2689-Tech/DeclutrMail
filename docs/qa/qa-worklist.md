...
| 🟡  | QA-triage-20260827-04 | P2 · **Tier 1 (billing)** | The Free-tier cap is one `::int` from inverting, and its spec runs on PGlite rather than the production driver                                                  | Codex round 2 clean on `8df9cbeb` (comment nits fixed in `f7f7929b`) — ready to push |      |
| 🟡  | QA-triage-20260827-05 | P2                        | D30's adaptive 5–12 queue size is dead code — no client ever calls `queue-size`, so everyone gets the hard max 12                                               | Codex round 2 clean on `8df9cbeb` (comment nits fixed in `f7f7929b`) — ready to push |      |
| 🟡  | QA-triage-20260827-06 | P2                        | The Triage empty state says new decisions arrive after a sync; the queue refills from already-scored rows with no sync                                          | Codex round 2 clean on `8df9cbeb` (comment nits fixed in `f7f7929b`) — ready to push |      |
| 🟡  | QA-triage-20260827-07 | P2                        | One measurement, two names on the same card: the row says "marked read", the tile and bullet say "read rate"                                                    | Codex round 2 clean on `8df9cbeb` (comment nits fixed in `f7f7929b`) — ready to push |      |
| 🟡  | QA-triage-20260827-08 | P2                        | "You'll see the affected email before anything changes" names Keep first, and Keep has no preview by design (D40)                                               | Codex round 2 clean on `8df9cbeb` (comment nits fixed in `f7f7929b`) — ready to push |      |
| 🟡  | QA-triage-20260827-09 | P2                        | The undo deadline renders in UTC in the toast and in the reader's zone in the preview, two clicks apart                                                         | Codex round 2 clean on `8df9cbeb` (comment nits fixed in `f7f7929b`) — ready to push |      |
| 🟡  | QA-triage-20260827-10 | P2                        | Two stat tiles are windowed and two are not, with nothing saying so; at 375px "90D" orphans onto its own line                                                   | Codex round 2 clean on `8df9cbeb` (comment nits fixed in `f7f7929b`) — ready to push |      |
| 🟡  | QA-triage-20260827-11 | P2                        | The preview's footer — reversibility line, Cancel, confirm — sits below the fold on a 375px phone                                                               | Codex round 2 clean on `8df9cbeb` (comment nits fixed in `f7f7929b`) — ready to push |      |
...
|     | id                  | sev | one line                                                                                                                                                                  | status                                                      | PR  |
| --- | ------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | --- |
| 🟡  | QA-undo-20260828-01 | P1  | Activity's own verb-count tiles include actions the user already undid while a different tile row on the same page correctly excludes them — no label says which is which | Approved this session — Codex round 2 clean — ready to push |     |
| 🟡  | QA-undo-20260828-02 | P2  | `/activity`'s stat row ships the desktop 5-column grid before hydration at 375px, so "UNSUBSCRIBES" and "KEPT" briefly overprint each other                               | Approved this session — Codex round 2 clean — ready to push |     |
...
| 🟡  | QA-undo-20260828-04 | P2  | Delete's own verb name disappears across 6 result surfaces, and its undo deadline repeats the two-clock mechanism already filed on `triage`, now on a second surface      | Approved this session — Codex round 2 clean — ready to push |     |
