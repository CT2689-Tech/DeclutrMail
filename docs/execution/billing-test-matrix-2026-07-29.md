...
| B2  | Start checkout, then in a **second browser profile** try again                                                                                                                                                     | `CHECKOUT_IN_FLIGHT` — **PASSED 2026-07-31 (409 while the claim is held)**                                                                                                                                                                                                                                                                                               |
| B3  | From B2, click the release. **D249 amended:** the button now renders only AFTER `POST /api/billing/reconcile` ran — the server asks the provider first; a found payment grants instead of releasing                | `DELETE /api/billing/checkout/pending` → `{released:true}`; checkout allowed again — **PASSED 2026-07-31: `{released:true}`, checkout reopened. The amended gating works — the release appears ONLY after reconcile answers `none_found`; on a DECLINED card reconcile answered `payment_in_progress` and correctly withheld it, since a decline is not an abandonment** |
...
| B8  | Abandon a checkout, wait past the **30-minute TTL**, retry checkout                                                                                                                                                | Succeeds — **no sweep needed.** The claim upsert reclaims any row whose `expires_at` has passed — **PASSED 2026-07-31 (201 + fresh session on an expired claim, no sweep involved)**                                                                                                                                                                                     |
| B9  | After B8, restart the worker (§0.5). **D249 amended:** the sweep now deletes only rows expired **>7 days** — a freshly-expired row is RETAINED so its `provider_ref` stays reachable for stale-lock reconciliation | Row expired <7d survives the sweep (still not a lock — B8's reopener is the TTL); `pendingCheckoutsCleared` counts only >7d rows — **PASSED 2026-07-31: planted one claim expired 1 day and one expired 8 days; the sweep deleted only the 8-day row, `pendingCheckoutsCleared: 1`, and the fresh one survived**                                                         |
...
| #   | Step                                                                                            | Expect                                                                                                                                                                                                                                                                                          |
| --- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1  | Buy Plus with a card that declines at checkout                                                  | No subscription row; no tier grant; legible error, not a raw 5xx — **PASSED 2026-07-31 (founder, sandbox decline card `4000…0002`: Paddle's own legible message in the overlay, 0 subscription rows, tier stayed `free`; the pending claim is HELD, which is B4 by design)**                    |
...
| #   | Step                                                                           | Expect                                                                                                                                                                                                            |
| --- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G1  | On Free, read `/me`                                                            | `tier=free`, `cleanupRemaining`, `cleanupResetsAt` = signup anniversary — **PASSED 2026-07-31 (`GET /api/auth/me`: free, 47/50, resets 2026-08-27 against a 2026-05-27 signup — a true monthly anniversary)**     |
...
| G5  | Open a Pro-only screen on Free                                                 | Tier gate / `ACTION_TIER_REQUIRED` — **PASSED 2026-07-31 (`/api/screener/queue` → 402 `PRO_FEATURE_REQUIRED`; `/autopilot/rules` stays 200 by design — capability-exempt so the entitlement preview can render)** |
...
| #   | Step                                                 | Expect                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| --- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| H1  | Refund a sandbox purchase in Paddle                  | `adjustment.created` lands; tier drops; `cancel_source='refund'` — **PASSED 2026-07-31: signed `adjustment.created`/`refund` sets `cancel_source='refund'`, `cancel_at_period_end=t` and `entitlement_ends_at=current_period_end`; the tier drops when that deadline passes. RE-RUN 2026-07-31 after the founder decision: a full refund now drops the tier IMMEDIATELY (it used to hold to `current_period_end`, which on an annual plan returned the whole charge and granted the rest of the year — measured at `entitlement_ends_at = 2027-07-31`). Chargebacks unchanged: instant, reviewed and kept. A PARTIAL refund does not end the plan at all — see H8**                                                                                                                                                                                                                                                                                                                                                          |
| H2  | **After H1, replay/receive any later renewal event** | Tier does **NOT** come back. A refund is a LOCAL verdict and must survive every later provider payload — this re-grant was the known gap 0051 closed — **PASSED 2026-07-31 (corrected). The tier DOES stay gone: `entitlement_ends_at` survives every later payload and is what the recompute reads — verified by pushing the deadline past after a replayed renewal and watching the workspace drop to `free`. An earlier stamp called this a launch-blocking failure; that was diagnosed off `cancel_at_period_end` without checking `entitlement_ends_at`. The two smaller real defects it exposed are now FIXED (`fix/d118-refund-verdict-provider-cancel`): `cancel_at_period_end` is pinned under a local verdict so `/billing` keeps saying the plan is ending, and the reconciliation sweep now schedules the cancel at Paddle so the provider stops billing. Re-smoked 2026-07-31: renewal echo left `cape=t`; worker sweep reported `verdictsEnforced: 1` and Paddle showed `scheduled_change: {action: cancel}`** |
| H3  | Simulate a chargeback                                | `cancel_source='chargeback'`, same stickiness as H2 — **PASSED 2026-07-31: signed `adjustment.created`/`chargeback` set `cancel_source='chargeback'`, `entitlement_ends_at=now()` and dropped `workspaces.tier` to `free` in the same transaction. The row then reads as non-backing on `/billing` (row tier pro vs entitlement free), which is the honest surface — the account IS on Free**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| H8  | **Partial** refund on a live subscription            | The plan is NOT ended — a goodwill part-refund is not an exit — **PASSED 2026-07-31: `adjustment.created`/`refund` with an item of type `partial` answers `{"status":"ignored"}`, leaves `cancel_source`/`entitlement_ends_at`/`cancel_at_period_end` untouched, and logs `billing.paddle.partial_refund_ignored`. Keyed on ITEM types, not the adjustment's own `type`, which Paddle defaults to `partial` even for a dashboard full refund**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| H9  | Win a dispute (`chargeback_reverse`)                 | The customer gets their plan back — **PASSED 2026-07-31: after a chargeback (tier `free`), a signed `chargeback_reverse` cleared `cancel_source` and `entitlement_ends_at` and restored `ws_tier=pro`. Over a REFUND verdict it correctly did nothing (`cancellation_revoked:no_verdict`). `cancel_at_period_end` is left set on purpose — the sweep may already have cancelled at Paddle, and with the verdict gone the D118 "Keep my subscription" button renders and works**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| H10 | Provider CONTRADICTS a local verdict                 | A rejected refund / reversed chargeback must lift the verdict, not strand a paying customer — **UNIT-TESTED ONLY 2026-07-31. Reaching it live needs Paddle to hold a rejected refund or reversed chargeback; sandbox auto-approves refunds, and creating a real adjustment on the dev subscription would permanently mark it refunded in the fixture the founder tests against. `settledCancellationCause` returns `refuted` and the sweep lifts the verdict through the projector**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
