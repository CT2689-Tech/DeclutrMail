...
## 2026-09-20 — The preview pane never ran a mount effect; a real browser did
**Context:** Persisting the Senders grid/table choice (`zustand/persist`, `skipHydration`, rehydrate in a mount effect). Unit + screen tests were green; the live smoke in the app's Browser pane showed storage = `table` but the page rendering grid after every reload.
**Finding:** The pane's tab is `visibilityState: 'hidden'` and the mount effect did not fire on full page loads there at all — SSR'd rows rendered and clicks worked, so the page looked alive. It fired exactly once, right after a file edit (Fast Refresh remounts), which read as "flaky". Two probes lied as well: `javascript_tool` runs in an isolated world and cannot see page `window.*` globals, and the console reader collapses repeated identical lines. In the Playwright MCP browser (`visible`), the same build restored the layout on navigate, reload, and after toggling both ways. Separately, the smoke surfaced something the tests could not: Senders is server-rendered WITH data, so a table user sees the server-painted grid until hydration — a product trade-off (cookie vs brief flicker) that went to the founder.
**Rule (provisional):** For anything driven by a mount effect, a red result in the Browser pane is UNVERIFIED, not failed — confirm in a visible browser, and prove "did the effect run" with a DOM marker, never a window global or a console line. And when persisting a client preference on a server-rendered route, check the served HTML first: if the preference changes the first paint, storage alone cannot prevent a flash.
**Distillation trigger:** promote to CLAUDE.md §8 "Known false-positive/negative traps" if a third effect-driven smoke misreads in the pane (the visibility entry there already covers refetchInterval / IntersectionObserver).
...
## 2026-09-20 — A server read of "what is still running" replaces three client bookkeeping problems
**Context:** Bottom-panel progress line for Archive/Later/Delete (founder: "one pill", Apple-simple).
**Finding:** Feeding the pill from `GET /api/actions/active` (groups = `coalesce(composite_id, id)`, the same partition the undo tray and batch status use) instead of each screen publishing its local job state gave reload- and navigation-survival for free, and made "a job nobody on this page started" a non-case. Two details mattered: (1) list a group for ~60s AFTER it ends (`running: false`) — otherwise a job quicker than one poll, or one whose screen unmounted, ends with nobody told; (2) re-read after ANY successful mutation via the MutationCache rather than per-hook invalidation, so a future action path cannot forget. The shared tray tests are SSR-only, so pill interactions are tested from apps/web.
**Rule (provisional):** For "is something happening?" UI, prefer one small server read keyed like the thing it turns into, plus a short settled-grace window, over per-screen client state.
**Distillation trigger:** promote if a second feature (Autopilot runs, sync) adopts the same read-with-grace shape.

## 2026-09-21 — A whole-app redesign fans out cleanly when ownership is by directory and one agent owns the chrome
**Context:** Founder-approved "Apple-simple" redesign of every app screen + the marketing homepage in one session: nine agents in ONE worktree, each owning a feature directory, all reading one shared design brief (type scale only, mono for numerals, one accent, motion tokens, copy budgets, "keep your `<ScreenIntro>` element").
**Finding:** Three things made it work. (a) Turning `ScreenIntro` into an invisible registration (renders `null`, feeds one top-bar `?` popover) removed 13 always-open intro cards with ZERO per-screen edits — so the cross-cutting change never collided with the per-screen agents. (b) Cross-agent contracts were one line each and named a file + signature (`SenderDetailPane({ senderId, onClose })`), created first as a thin wrapper. (c) Agents cannot see the app: every one reported "not visually checked", and during the fan-out the dev server 500'd on every authed route because one agent had deleted a file another still imported. The visual pass and the structural gates had to run AFTER the last agent, and they found real defects green suites did not (see MISTAKES 2026-09-21). Also: plain `rm` of tracked files is blocked for agents in this environment; `git rm` works and is recoverable.
**Rule (provisional):** For a cross-cutting UI change, first look for the single component whose behaviour can change instead of its N call sites. Budget the orchestrator's time for a post-fan-out visual + gate pass — it is not optional.
**Distillation trigger:** promote to CLAUDE.md §8 if a second multi-agent UI sweep hits the same "green per agent, broken at the joins" shape.

## 2026-09-25 — An unprepared statement is two round trips; count statements, not rows
**Context:** Profiling the 2026-09-25 incident (482s mailbox-lock hold for 8,692 label records).
**Finding:** (1) postgres.js with `prepare: false` — required by the Supabase transaction pooler — sends Parse+Describe, waits for the parameter types, then Bind+Execute (`describeFirst` in `connection.js`), so every parameterized statement costs TWO round trips. Production's 55ms per record implies ~27ms RTT Cloud Run → Supabase. A local TCP proxy adding 12ms each way reproduced production within 6% (512s vs 482s); at 0 RTT the same apply took 2.7s, so server time was never the cost. (2) BullMQ 6 ships `deduplication: { id, keepLastIfActive: true }`: at most one waiting and one active job per id, and adds that land while the job runs are stored and enqueued ONCE when it finishes — in Lua, so racing producers cannot both win. `add()` returns the absorbing job's id on a dedup hit, which is how a producer tells created from absorbed; with a unique jobId per add, a finished job never swallows the next one. (3) The stored follow-up is keyed by the DEDUP id and carries the absorbed add's custom jobId, and it is enqueued when the NEXT job of that dedup id finishes — not necessarily the one that absorbed it. Retrying an add with the same jobId therefore collides with that follow-up (one push ran twice under one id in our stuck-holder test); every add attempt needs a fresh id.
**Rule (provisional):** For DB-bound worker latency, multiply statements by 2 x RTT before opening a query plan, and benchmark through a latency proxy, not a local socket. For per-mailbox work triggered by external events, enqueue through `addCoalescedJob`.
**Distillation trigger:** promote to CLAUDE.md §8 "Performance measurement discipline" if a second lock-hold or latency incident traces to per-row statements.

## 2026-09-25 — Shrinking a lock's scope moves its READS, not its writes
**Context:** Bounding the Autopilot-sweep and Later-wake mailbox-lock holds (one hold per sweep or pass → one per match or sender), the follow-up to the 2026-09-25 incident.
