...
**Finding:** BullMQ 5.77.0 rejects a custom `jobId` containing `:` (`"Custom Id cannot contain :"` — it reserves `:` as its Redis key separator). My verb-namespaced keys (`archive:<uuid>`, `revert:<token>`) tripped it → `queue.add` threw → my catch marked the row `failed` + returned 503. The PGlite integration tests used a FAKE queue that recorded `add()` WITHOUT BullMQ's jobId validation, so the bug was invisible until a live Redis. Fix: separator `-` not `:`, normalize `:`→`-` in any client key, AND teach the fake queue to throw on `:` so the test now has the same teeth as prod.
**Rule (provisional):** A fake/mock standing in for an external service must replicate its REJECTION rules, not just its happy path — otherwise integration tests pass on inputs prod refuses. For BullMQ specifically: jobIds must be colon-free.
