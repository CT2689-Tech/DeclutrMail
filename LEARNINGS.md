...
## 2026-05-29 — Regenerating `atlas.sum` by hand when Atlas can't be installed
**Context:** Adding migration 0016 (D181 security_events) in a container where the Atlas CLI couldn't be installed (network policy 403) and there's no DB/docker — so `atlas migrate hash` was unavailable, but a stale/missing sum entry would break tooling.
**Finding:** Atlas's `atlas.sum` is a deterministic, replicable hash (no secret key): per `.sql` file (sorted, `.rollback` excluded) the entry is `h1:` + `base64(sha256(filename_bytes + file_content_bytes))`; the first line's total is `h1:` + `base64(sha256( concat over files of (filename + filehash_b64) ))` with NO separators. Verified by reproducing existing entries exactly (a 12-line python script). Separately discovered the committed sum was already STALE for 0001–0015 (only 0000 matched its content). I first appended only 0016 (surgical), assuming `atlas migrate lint --latest 1` didn't verify the sum — **that assumption was wrong**: CI's `atlas migrate lint` failed fast (~19s, before the dev-DB lint analysis) on a checksum mismatch. `migrate lint` DOES enforce dir integrity. Fix: a full `atlas migrate hash`-equivalent regeneration (all per-file hashes from current bytes + correct total), which also corrected the pre-existing drift.
**Rule (provisional):** When Atlas is unavailable, you can still produce a correct `atlas.sum` with the formula above — but you must regenerate ALL entries (a partial/append-only sum fails `atlas migrate lint`'s integrity check). `atlas migrate lint` validates the checksum before linting, so the sum must match every migration file's current bytes, not just the one you added.
**Distillation trigger:** promote to CLAUDE.md §4 (migration workflow) if a 2nd migration PR needs hand-hashing, or if the sum-staleness bites CI again.
