...
**Finding:** Atlas's `atlas.sum` is a deterministic, replicable hash (no secret key): per `.sql` file (sorted, `.rollback` excluded) the entry is `h1:` + `base64(sha256(filename_bytes + file_content_bytes))`; the first line's total is `h1:` + `base64(sha256( concat over files of (filename + filehash_b64) ))` with NO separators. Verified by reproducing existing entries exactly (a 12-line python script). Separately discovered the committed sum was already STALE for 0001–0015 (only 0000 matched its content) — CI's `atlas migrate lint --latest 1` does not enforce sum integrity, so the drift was invisible. Logged as a FOUNDER-FOLLOWUP; left the stale lines untouched (surgical) and appended only 0016 + recomputed the total.
**Rule (provisional):** When Atlas is unavailable, you can still produce a correct `atlas.sum` entry with the formula above — but prefer running the real `atlas migrate hash` in CI/locally when possible. Don't "fix" pre-existing stale hashes in an unrelated PR; append your entry, recompute the total, and flag the drift. `atlas migrate lint` ≠ sum validation; add `atlas migrate validate` if integrity must be enforced.
**Distillation trigger:** promote to CLAUDE.md §4 (migration workflow) if a 2nd migration PR needs hand-hashing, or if the sum-staleness bites CI.
