...
## 2026-07-09 — cloud-smoke.sh: `runuser` + `localhost` cookie-jar traps in locked-down sandboxes
**Context:** Pre-launch full-product smoke in a Cursor cloud agent (no Docker; Postgres via local binary).
**Finding:** (1) `runuser -u postgres` fails for non-root callers on some images even when `sudo -u postgres` works — `cloud-smoke.sh up` then reported "API up" against a stale/empty readiness probe while Postgres never started. (2) Nest may bind IPv4 while `curl http://localhost` prefers `::1`, so the readiness loop can false-pass on a leftover listener and the Netscape cookie jar keyed to `localhost` won't send cookies to `127.0.0.1` (or vice versa) → every authed call 401s with "Missing session." (3) If PG init creates `/tmp/dmlogs` as `postgres`, the invoking user can't write `cookies.txt`.
**Rule (provisional):** Prefer `127.0.0.1` for API smoke curls + cookie jar host; fall back `sudo -u postgres` when `runuser` is unusable; ensure the log/cookie dir is writable by the invoker before `login`.
**Distillation trigger:** promote into `scripts/cloud-smoke.sh` comments / CLAUDE.md §8 cloud-smoke row if a second session hits the same 401-after-302 pattern.
